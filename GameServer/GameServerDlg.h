...
typedef std::vector<TimerEventList> TimerEventListShow;
...
public:
	...
	void TrovoLive();
	std::string server_itemorg, server_skillmagic, server_zones, server_itemsell_table;
	...
	bool LoadBotInfoTable(bool m_bStatus);
	...
	bool LoadRightClickExchangeTable();
	bool LoadRightExchangeTable();
#endif
	bool MapFileLoad();
	bool LoadNoticeData();
	...
	static uint32 THREADCALL Timer_AutoBotMoving(void* lpParam);
#if 0
	static uint32 THREADCALL Timer_AutoBotSpawn(void* lpParam);
#endif
	void BotHandlerMainTimer();
	void BotAutoHandlerMainTimer();
	void BotHandlerMainMerchantTimer();
	...
	uint16 SpawnEventBotFarm(int Minute, uint8 byZone, float fX, float fY, float fZ, uint8 minlevel, uint8 sPartyLider, uint8 sGenie, uint8 sClass);
	uint16 SpawnEventBotMoveProcess(int Minute, uint8 byZone, float fX, float fY, float fZ, uint8 minlevel);
	uint16 SpawnEventBotPk(int Minute, uint8 byZone, float fX, float fY, float fZ, uint8 minlevel, uint8 sNation, uint8 sClass, CUser* pUser = nullptr);
	uint16 SpawnUserBot(int Minute, uint8 byZone, float fX, float fY, float fZ, uint8 Restipi, uint8 minlevel = 1, int16 direction = 0, uint32 SaveID = 0, uint8 Class = 0, _bot_merchant _merchant = _bot_merchant{});
	void AutoSpawnPKBots();
	void MerchantBotAddSet(uint32 index, uint8 type);
	uint16 SpawnLoginBot(float fX, float fY, float fZ, uint8 type, uint8 byZone);
	...
	uint16 m_sMaxSpawnCount;
	uint16 m_sMaxBotFinish;
	uint16 m_sBotStepCount;
	ULONGLONG m_sBotRespawnTick;
	uint32 m_sBotTimeNext;
	uint8 m_sBotAutoType;
	int m_sBotAutoX;
	int m_sBotAutoZ;
	uint8 m_byZoneID;
	...
	bool m_bMerchantBotStatus, m_sLevelMerchantStatus, m_sLevelRewardStatus;
	...
	bool m_sSummonMessage;
	std::string m_sSummonGameMaster;

	bool m_bChaosStoneRespawnStatus;
	...
	void TempleSoccerEventGool(int8 nType, int8 sZone);
	...
	INLINE bool isUnderTheCastleActive() { return pUnderTheCastle.isActive; }
	...
	void CastleSiegeWarfarePrepaOpen();
	void CastleSiegeWarfareWarOpen();
	void CastleSiegeWarfareTools(bool Notice = false,
		...
		bool town = false,
		bool reward = false);
	void SiegeWarfareMainTimer();
	void CastleSiegeWarfareRawNotice(uint8 NoticeType, int32 Time);
	void CastleSiegeWarfareReset();
	void CastleSiegeWarfareClose();
	...
	IDMapBotList						m_sMapBotListArray;
	...
	LoadUpgrade							m_sLoadUpgradeArray;
	UpgradeSettings						m_sUpgradeSettingArray;
	...
	UserNameRankMap						m_BotKarusPersonalRankMap;
	UserNameRankMap						m_BotElmoPersonalRankMap;
	UserNameRankMap						m_BotKarusKnightsRankMap;
	UserNameRankMap						m_BotElmoKnightsRankMap;
	...
	TempleSoccerEventRoomList			m_TempleSoccerEventRoomList;

	StartPositionArray					m_StartPositionArray;
	ServerResourceArray					m_ServerResourceArray;
	BotSaveDataArray					m_bBotSaveDataArray;
	...
	//SoccerEventUserMap					m_SoccerEventUserMap;
	//TempleEventJuraidRoomList			m_TempleEventJuraidRoomList;
	//TempleEventBDWRoomList			m_TempleEventBDWRoomList;
	//TempleEventChaosRoomList			m_TempleEventChaosRoomList;
...
#if(GAME_SOURCE_VERSION == 2369 || GAME_SOURCE_VERSION == 1098)
	HShieldSoftwareRightClickExchangeArray	s_HShieldSoftwareRightClickExchangeArray;
	HShieldSoftwareRightExchangeArray		s_HShieldSoftwareRightExchangeArray;
#endif
	...
	BotNameMap	m_sBotCharackterNameArray;
	std::recursive_mutex m_AntiAfkListLock;
	AntiAfkList m_AntiAfkList;
	GmList m_GmList;

	std::recursive_mutex m_sTimeShowEventListLock;
	TimerEventListShow m_sTimeShowEventList;

	bool timershowreset;

	std::recursive_mutex	m_accountNameLock,
		m_characterNameLock,
		m_questNpcLock, m_sBotCharackterNameLock,
		m_sBotMapListLock;
	...
	KOSocketMgr<CUser> m_sClientSocket;
	bool m_LoginServerConnected;

	void SendYesilNotice(CUser* pUser, std::string sHelpMessage); //offline merchant i�in eklendi
...
private:
	...
	bool HandleWarpTime(CBot* pBot, DWORD currentTime);
	void HandleNormalBot(CBot* pBot, time_t fTime2, DWORD currentTime);
	void HandleLoginBot(CBot* pBot, time_t fTime2, DWORD currentTime);
	void HandleGameStartMagic(CBot* pBot);
	void HandleBotActions(CBot* pBot, time_t fTime2, DWORD currentTime);
	void HandleBotReply(CBot* pBot, DWORD currentTime);
	void HandleBotState(CBot* pBot);
	void HandleBotSpeedCheck(CBot* pBot, time_t fTime2);
	void SetBotSpeed(CBot* pBot);
	void RemoveBots(const std::vector<CBot*>& botsToRemove);
public:
