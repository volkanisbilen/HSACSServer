...
public:
	...
	void	CastleSiegeWarfareMonumentProcess(CUser *pKiller);
...
public:
	void Dead(Unit* pKillerUnit = nullptr, bool bSendDeathPacket = true);
	...
	bool	IsCloseTarget(CBot* pUser, int nRange);
	void	ChangeTarget(int nAttackType, CUser *pUser);
	void	ChangeTarget(int nAttackType, CBot* pUser);
	...
	bool	IsDamagedNpcList(CNpc* pNpc);
	bool	IsDamagedBotList(CBot* pUser);
...
public:
	...
	short GetDamage(CBot* pTarget, _MAGIC_TABLE pSkill = _MAGIC_TABLE(), bool bPreviewOnly = false);
