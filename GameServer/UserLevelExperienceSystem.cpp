...
void CUser::ExpChange(std::string descp, int64 iExp, bool bIsBonusReward)
{
	...
	if (iExp > 0)
	{
		if (!bIsBonusReward)
		{
			...
			if (m_bFlamelevel > 0 && m_bFlamelevel <= 3 && g_pMain->pBurningFea[m_bFlamelevel - 1].exprate)
				FinalExp += (TempExp * g_pMain->pBurningFea[m_bFlamelevel - 1].exprate) / 100;
	...
	result << uint8(0x04) << m_iExp; // NOTE: Use proper flag
	Send(&result);
}
...
void CUser::LevelChange(uint8 level, bool bLevelUp)
{
	...
	result << uint32(GetSocketID())
		<< GetLevel() 
		<< m_sPoints 
		<< m_bstrSkill[(uint8)SkillPointCategory::SkillPointFree]
		<< m_iMaxExp 
		<< m_iExp
		<< m_MaxHp 
		<< m_sHp
	...
	if (isInParty())
	{
		...
		result << uint8(PARTY_LEVELCHANGE) << uint32(GetSocketID()) << GetLevel();
...
bool CUser::JackPotNoah(uint32 gold)
{
	...
	newpkt << uint16(740) << uint16(0) << uint32(0) << uint16(xrand) << uint32(GetSocketID());
...
bool CUser::JackPotExp(int64 iExp)
{
	...
	newpkt << uint32(GetSocketID()) << xrand << m_bExpEvent;
	SendToRegion(&newpkt, nullptr, GetEventRoom());
