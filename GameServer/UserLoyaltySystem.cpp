...
void CUser::SendLoyaltyChange(std::string placeowned, int32 nChangeAmount /*= 0*/, bool bIsKillReward /* false */, bool bIsBonusReward /* false */, bool bIsAddLoyaltyMonthly /* true */)
{
	...
	else
	{
		if (bIsKillReward)
		{
			...
			if (m_bFlamelevel > 0 && m_bFlamelevel <= 3 && g_pMain->pBurningFea[m_bFlamelevel - 1].droprate)
				nChangeAmount = nChangeAmount * (100 + g_pMain->pBurningFea[m_bFlamelevel - 1].droprate) / 100;
