...
void CUser::SelectWarpList(Packet & pkt)
{
	...
	if (isInKarusEslant((uint8)zoneid) || isInElmoradEslant((uint8)zoneid))
	...
	if (m_bZone == pWarp->sZone) { m_bZoneChangeSameZone = true; Packet result(WIZ_WARP_LIST, uint8(2)); result << uint8(1) << uint8(0); Send(&result); }
