...
int CNpcThread::_Engine()
{
	try
	{
		...
		{
			deleted.clear();
			if (g_pMain->m_bNpcTheardReload) { Sleep(200); continue; }
...
bool CGameServerDlg::_CreateNpcThread()
{
	...
	foreach(HSACSXSoftjanekdominik, mZoneList)
	{
		CNpcThread* pNpcThread = new CNpcThread((*HSACSXSoftjanekdominik)->m_nZoneNumber);
		if (!m_arNpcThread.PutData((*HSACSXSoftjanekdominik)->m_nZoneNumber, pNpcThread)) { delete pNpcThread; return false; }

		std::string threadid = std::to_string((*HSACSXSoftjanekdominik)->m_nZoneNumber);
		if ((*HSACSXSoftjanekdominik)->m_nZoneNumber < 10) threadid = string_format("0%d", (*HSACSXSoftjanekdominik)->m_nZoneNumber);
		++counter;
	...
	for (uint16 j = 0; j < bNumNpc; j++)
	{
		...
		else if (pNpc->GetZoneID() == ZONE_BORDER_DEFENSE_WAR)
		{
			if (pNpc->GetEventRoom() > 0 && pNpc->GetEventRoom() <= MAX_TEMPLE_EVENT_ROOM)
			{
				...
				if (pNpc->GetProtoID() == ALTAR_OF_MANES)
				{
					if (!pRoomInfo->pAltar)
		...
		else if (pNpc->GetZoneID() >= ZONE_MORADON && pNpc->GetZoneID() <= ZONE_MORADON5)
		{
			if (pNpc->GetType() == NPC_SOCCER_BAAL)
			{
				auto pRoomInfo = g_pMain->m_TempleSoccerEventRoomList.GetData(pNpc->GetZoneID());
				if (pRoomInfo != nullptr)
					pRoomInfo->m_sNpcID = pNpc->GetID();
			}
		}
...
void CNpcThread::_LoadAllObjects()
{
	...
	auto m_sObjectEventArray = g_pMain->m_ObjectEventArray.m_UserTypeMap;
	g_pMain->m_ObjectEventArray.m_lock.unlock();

	foreach(itr, m_sObjectEventArray)
	{
		auto* pEvent = itr->second;
		if (pEvent == nullptr 
			|| pEvent->sZoneID != m_sZoneID) 
			continue;
...
bool CNpcThread::_AddObjectEventNpc(_OBJECT_EVENT* pEvent, uint16 nEventRoom /* = 0 */, bool isEventObject /* false */)
{
	...
	bool object = { pEvent->sType == OBJECT_GATE 
		|| pEvent->sType == OBJECT_GATE2 
		|| pEvent->sType == OBJECT_GATE_LEVER
		|| pEvent->sType == OBJECT_ANVIL 
		|| pEvent->sType == OBJECT_ARTIFACT
	...
	uint8 m_byDirection /* = 0 */, uint8 m_bMoveType /* = 0 */, uint8 m_bGateOpen/* = 0 */, uint16 m_nSummonSpecialType/* = 0 */, uint32 m_nSummonSpecialID/* = 0 */)
{
	...
	for (int i = 0; i < m_sCount; i++)
	{
		...
		if (m_bIsMonster)
			proto = g_pMain->m_arMonTable.GetData(m_sSid);
		else 
			proto = g_pMain->m_arNpcTable.GetData(m_sSid);
		...
		if (m_nSummonSpecialID == 1) 
			pNpc->e_stype = e_summontype::m_MonsterStoneBoss;
...
void CNpcThread::ResetAllNPCs()
{
	...
	auto m_sNpcArray = m_arNpcArray.m_UserTypeMap;
	m_arNpcArray.m_lock.unlock();

	foreach(itr, m_sNpcArray)
	{
...
void CNpcThread::RemoveAllNPCs(uint8 eventroom /* = 0*/)
{
	...
	foreach(itr, m_sNpcArray)
	{
		...
		if (pNpc == nullptr 
			|| !pNpc->m_bIsEventNpc 
			|| pNpc->isDead() 
			|| (eventroom > 0 && pNpc->GetEventRoom() != eventroom))
			continue;
...
void CNpcThread::ChangeAbilityAllNPCs(uint8 bType)
{
	...
	foreach(itr, m_sNpcArray)
	{
		...
		if (pNpc->GetType() > 10 
			&& (pNpc->GetNation() == (uint8)Nation::KARUS 
				|| pNpc->GetNation() == (uint8)Nation::ELMORAD))
...
void CNpc::HandleJuraidKill(CUser* pKiller)
{
	...
	if (!pRoomInfo->m_FinishPacketControl && ((mainMobKill == 13 && subMobKill == 60) || (GetProtoID() == DEVA_BIRD_SSID/* && door*/)))
	{
		...
		newpkt1 << uint32(0) << uint8(7) << uint64(0);
...
void CNpc::HandleJuraidGateOpen(uint16 pBridgeid, uint8 door, _JURAID_ROOM_INFO* pRoomInfo, Nation bNation)
{
	...
	result << uint8(InOutType::INOUT_IN) << uint32(pBridges->GetID());
