...
void CUser::HandleCapeChange(Packet & pkt)
{
	...
	if (sCapeID >= 0) {
		if (castnewcape) {
			pKnights->m_sCastCapeID = sCapeID;
			pKnights->m_sCastellanCape = true;
			uint32 CapeTime = opcode == 1 ? 14 : 15;
			pKnights->m_sCastTime = uint32(UNIXTIME + uint32(60 * 60 * 24 * CapeTime));
		}
	...
	if (!pKnights->isInAlliance()) {
		if (isKing()) {
			...
			if (pKnights->isCastellanCape())
				result << pKnights->m_sCastCapeID << pKnights->m_bCastCapeR << pKnights->m_bCastCapeG << pKnights->m_bCastCapeB;
			else
		...
		if (isKing()) {
			...
			if (pMainKnights != nullptr && pMainKnights->isCastellanCape())
				result << pMainKnights->m_sCastCapeID << pKnights->m_bCastCapeR << pKnights->m_bCastCapeG << pKnights->m_bCastCapeB;
			else
	...
	if (pKnights->isCastellanCape())
		result << uint8(1) << pKnights->GetID() << pKnights->m_sCastCapeID << pKnights->m_bCastCapeR << pKnights->m_bCastCapeG << pKnights->m_bCastCapeB << pKnights->m_sCastTime;
	else
...
void CKnights::ReqKnightCapeBonus()
{
	...
	auto m_sKnightsUser = m_arKnightsUser.m_UserTypeMap;
	m_arKnightsUser.m_lock.unlock();

	foreach(user, m_sKnightsUser)
	{
	...
	foreach(itr, mlist)
	{
		...
		if (isCastellanCape()) 
		{
			auto* pCape = g_pMain->m_KnightsCapeArray.GetData(m_sCastCapeID);
	...
	if (isInAlliance()) {
		...
		if (pMainClan->isCastellanCape()) {
			if (pAlliance->sMainAllianceKnights == GetID())
				result << GetID() << m_byFlag << pMainClan->m_sCastCapeID << pMainClan->m_bCastCapeR << pMainClan->m_bCastCapeG << pMainClan->m_bCastCapeB << uint8(0);
			else if (pAlliance->sSubAllianceKnights == GetID())
				result << GetID() << m_byFlag << pMainClan->m_sCastCapeID << m_bCastCapeR << m_bCastCapeG << m_bCastCapeB << uint8(0);
			else if (pAlliance->sMercenaryClan_1 == GetID() || pAlliance->sMercenaryClan_2 == GetID())
				result << GetID() << m_byFlag << pMainClan->m_sCastCapeID << uint32(0); // only the cape will be present
		}
		else {
			if (pAlliance->sMainAllianceKnights == GetID())
		...
		if (isCastellanCape())
			result << GetID() << m_byFlag << m_sCastCapeID << m_bCastCapeR << m_bCastCapeG << m_bCastCapeB << uint8(0);
		else
