...
#pragma region CUser::ReqHandleHSACSX(Packet &pkt)
void CUser::ReqHandleHSACSX(Packet &pkt) {
	uint8 opcode = pkt.read<uint8>();
	switch ((HSACSXOpCodes)opcode)
	{
...
void CUser::HandleItemReturn(Packet &pkt) {
	...
	Packet newpkt(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::PusRefund));
...
{
	if (m_PusRefundMap.IsEmpty())
		return;

	uint16 count = 0;
#if(__VERSION < 2369)
	Packet result(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::PusRefund));
	result << uint8(pusrefunopcode::listsend) << count;
#endif
	m_PusRefundMap.m_lock.lock();
	auto m_sPusRefundMap = m_PusRefundMap.m_UserTypeMap;
	m_PusRefundMap.m_lock.unlock();

	foreach(itr, m_sPusRefundMap)
	{
		auto* p = itr->second;
		if (!p 
			|| UNIXTIME > p->expiredtime) 
			continue;
#if(__VERSION < 2369)
		result << itr->first << itr->second->itemid << itr->second->itemprice << itr->second->expiredtime;
#endif
		count++;
	}
#if(__VERSION < 2369)
	result.ByteBuffer::put(2, count);
...
void CUser::PUSGiftPurchase(Packet &pkt) {
	...
	else if (sCodeType == 1)
	{
		...
		if (m_nTLBalance < totalCash) {
			g_pMain->SendHelpDescription(this, "Your tl amount is not enough.");
			return;
		}
	}
	else
	{
		g_pMain->SendHelpDescription(this, "Your complex error occurred in the system.");
		return;
	}
	...
	newpkt << sCodeType << item->ItemID << item->Price << name;
	g_pMain->AddDatabaseRequest(newpkt, this);
}