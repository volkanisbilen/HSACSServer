...
void CUser::HandleRankingPKZone(Packet& pkt)
{
	...
	std::vector<uint8> CZRankKarus;
	std::vector<uint8> CZRankHuman;
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++)
	{
		g_pMain->m_UserPlayerKillingZoneRankingArray[nation].m_lock.lock();
		auto m_UserPlayerKillingZoneRankingArray = g_pMain->m_UserPlayerKillingZoneRankingArray[nation].m_UserTypeMap;
		g_pMain->m_UserPlayerKillingZoneRankingArray[nation].m_lock.unlock();

		foreach(itr, m_UserPlayerKillingZoneRankingArray)
		{
		...
		if ((uint32)UserRankingSorted[nation].size() > 0)
		{
			foreach(itr, UserRankingSorted[nation])
			{
				...
				if (sCount <= 9)
				{
					Unit* pUnit = g_pMain->GetUnitPtr(pRankInfo->p_SocketID);
					if (pUnit == nullptr)
						continue;

					if (pUnit->isPlayer())
					{
						HandleUserRankingPKZone(pRankInfo, result);

						if (pUnit->GetNation() == uint8(Nation::ELMORAD))
							CZRankHuman.push_back(TO_USER(pUnit)->GetSymbol());
						else if (pUnit->GetNation() == uint8(Nation::KARUS))
							CZRankKarus.push_back(TO_USER(pUnit)->GetSymbol());
					}
					else if (pUnit->isBot())
					{
						HandleBotRankingPKZone(pRankInfo, result);

						if (pUnit->GetNation() == uint8(Nation::ELMORAD))
							CZRankHuman.push_back(TO_BOT(pUnit)->GetSymbol());
						else if (pUnit->GetNation() == uint8(Nation::KARUS))
							CZRankKarus.push_back(TO_BOT(pUnit)->GetSymbol());
					}
					else
						continue;
	...
	result << uint8(HSACSXOpCodes::PlayerRank);
	result << uint8(2) << uint16(CZRankHuman.size());
	foreach(itr, CZRankHuman)
		result << uint8(*itr);

	result << uint8(1) << uint16(CZRankKarus.size());
	foreach(itr, CZRankKarus)
		result << uint8(*itr);
...
#pragma region CUser::HandleUserRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet & result)
void CUser::HandleUserRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet& pkt)
{
	if (pRankInfo == nullptr)
		return;
	...
	pkt << pUser->GetName() << pUser->GetNation();

	CKnights* pKnights = g_pMain->GetClanPtr(pUser->GetClanID());
	if (pKnights == nullptr)
		pkt << uint16(0) << uint16(0) << (std::string)"";
	else
		pkt << pKnights->GetID() << pKnights->m_sMarkVersion << pKnights->GetName();

	pkt << pRankInfo->P_LoyaltyDaily;

	if (pRankInfo->P_LoyaltyPremiumBonus <= 999)
		pkt << pRankInfo->P_LoyaltyPremiumBonus;
	else
		pkt << uint16(999);

	
	pkt << pUser->GetLoyaltySymbolRank();

}
#pragma endregion

#pragma region CUser::HandleBotRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet & result)
void CUser::HandleBotRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet& pkt)
{
	...
	pkt << pBot->GetName() << pBot->GetNation();
...
void CUser::HandleRankingSpecialEvent(Packet & pkt)
{
	...
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++)
	{
		g_pMain->m_ZindanWarZoneRankingArray[nation].m_lock.lock();
		auto m_ZindanWarZoneRankingArray = g_pMain->m_ZindanWarZoneRankingArray[nation].m_UserTypeMap;
		g_pMain->m_ZindanWarZoneRankingArray[nation].m_lock.unlock();

		foreach(itr, m_ZindanWarZoneRankingArray)
		{
			if (itr->second 
				&& GetZoneID() == itr->second->z_Zone) 
				UserRankingSorted[nation].push_back(*itr->second);
		}
...
void CUser::HandleRankingBDW(Packet& pkt)
{
	...
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++)
	{
		...
		auto m_UserBorderDefenceWarRankingArray = g_pMain->m_UserBorderDefenceWarRankingArray[nation].m_UserTypeMap;
		g_pMain->m_UserBorderDefenceWarRankingArray[nation].m_lock.unlock();

		foreach(itr, m_UserBorderDefenceWarRankingArray)
		{
...
void CUser::HandleRankingChaosDungeon(Packet& pkt)
{
	...
	auto m_sUserChaosExpansionRankingArray = g_pMain->m_UserChaosExpansionRankingArray.m_UserTypeMap;
	g_pMain->m_UserChaosExpansionRankingArray.m_lock.unlock();

	foreach(itr, m_sUserChaosExpansionRankingArray)
		if (itr->second && m_bEventRoom == itr->second->c_EventRoom)
			UserRankingSorted.push_back(*itr->second);

	if (UserRankingSorted.empty()) 
		return;
...
uint16 CUser::GetPlayerRank(uint8 nRankType)
{
	...
	case RANK_TYPE_PK_ZONE:
	{
		...
		g_pMain->m_UserPlayerKillingZoneRankingArray[nRankArrayIndex].m_lock.lock();
		auto m_UserPlayerKillingZoneRankingArray = g_pMain->m_UserPlayerKillingZoneRankingArray[nRankArrayIndex].m_UserTypeMap;
		g_pMain->m_UserPlayerKillingZoneRankingArray[nRankArrayIndex].m_lock.unlock();
	...
	case RANK_TYPE_ZONE_BORDER_DEFENSE_WAR:
	{
		...
		g_pMain->m_UserBorderDefenceWarRankingArray[nRankArrayIndex].m_lock.lock();
		auto m_UserBorderDefenceWarRankingArray = g_pMain->m_UserBorderDefenceWarRankingArray[nRankArrayIndex].m_UserTypeMap;
		g_pMain->m_UserBorderDefenceWarRankingArray[nRankArrayIndex].m_lock.unlock();
...
{
	std::vector<uint16> m_UserList, m_BotList;
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++) 
	{
		g_pMain->m_UserPlayerKillingZoneRankingArray[nation].m_lock.lock();
		auto m_UserPlayerKillingZoneRankingArray = g_pMain->m_UserPlayerKillingZoneRankingArray[nation].m_UserTypeMap;
		...
		foreach(itrRank, m_UserPlayerKillingZoneRankingArray)
		{
			...
			if (pUnit->isPlayer())
				m_UserList.push_back(pRankInfo->p_SocketID);
			else if (pUnit->isBot())
				m_BotList.push_back(pRankInfo->p_SocketID);
		}
	}
...
void CUser::ShowKarusRankBoard()
{
	...
	auto m_KnightsRatingArray = g_pMain->m_KnightsRatingArray[KARUS_ARRAY].m_UserTypeMap;
	g_pMain->m_KnightsRatingArray[KARUS_ARRAY].m_lock.unlock();

	foreach(itr, m_KnightsRatingArray)
	{
		if (itr->second == nullptr)
			continue;
...
void CUser::ShowElmoradRankBoard()
{
	...
	if (pKingKnights == nullptr)
		...
		<< uint16(0)
		<< pKingKnights->GetID() 
		<< uint16(pKingKnights->m_sMarkVersion)
		<< pKingKnights->GetName()
		<< uint8(pKingKnights->m_byFlag)  // clan flag
		<< uint8(pKingKnights->m_byGrade); // clan grade
	...
	auto m_KnightsRatingArray = g_pMain->m_KnightsRatingArray[ELMORAD_ARRAY].m_UserTypeMap;
	...
	foreach(itr, mlist) 
	{
		...
		result << index++ 
			...
			<< pKnights->GetName()
			<< uint8(pKnights->m_byFlag) 
			<< uint8(pKnights->m_byGrade);
		sCount++;
	}
