...
#define UNDER_THE_CASTLE_MAX_USER	80
#define THE_CASTELLAN_MAX_USER		80
...
struct _ITEM_RIGHT_CLICK_EXCHANGE
{
	uint32	sItemID;
	uint8	sSobCode;
};
...
struct _ITEM_RIGHT_EXCHANGE
{
	...
	uint32 nExpirationTime[ITEMS_RIGHT_CLICK_EXCHANGE_GROUP];
};
#endif
...
enum ItemMovementType
{
	...
	ITEM_SLOT_INVEN_TO_MBAG = 14 // Inventory -> Magic Bag Slot
};
...
enum ItemSlotType
{
	...
	ItemSlotCospreTalisman		= 113,
	ItemSlotCospre0Emblem		= 114,
	ItemSlotCospre1Emblem		= 115
};
...
{
	uint32_t sIndex;
	uint8_t Facebook, Discord, Live, DiscordToken;
	std::string FacebookURL, DiscordURL, LiveURL, ResellerURL, DiscordTokenID;
	...
	{
		sIndex = 0;
		Facebook = Discord = Live = DiscordToken = 0;
		FacebookURL.clear();
		ResellerURL.clear();
		DiscordURL.clear();
		DiscordTokenID.clear();
		LiveURL.clear();
	}
};
...
struct _SOCCER_STARTED_EVENT_USER
{
	std::string strUserID;
	bool isPrizeGiven;
	bool isLoqOut;

	_SOCCER_STARTED_EVENT_USER()
	{
		void initiliaze();
	}
...
struct _SOCCER_STATUS_INFO
{
	std::recursive_mutex SoccerUserListLock;
	typedef std::map<std::string, _SOCCER_STARTED_EVENT_USER>  SeccerUserList;
	SeccerUserList m_SeccerUserList;
	...
	{
		Guard lock(SoccerUserListLock);
		return (uint8)m_SeccerUserList.size();
	}

	void Initialize()
	{
		SoccerUserListLock.lock();
		m_SeccerUserList.clear();
		SoccerUserListLock.unlock();

		m_SoccerActive = false;
		m_SoccerTimer = false;
		m_SoccerSocketID = -1;
		m_SoccerTimers = 0;
		m_SoccerTime = 0;
		m_SoccerRedColour = 0;
		m_SoccerBlueColour = 0;
		m_SoccerBlueGool = 0;
		m_SoccerRedGool = 0;
		m_sNpcID = -1;
	}
...
{
	uint16 m_bKillCount[3];
	bool m_bFinish;
	uint8 m_bUserStatus;
	_CR_USER_LIST()
	{
		m_bFinish = false;
		memset(&m_bKillCount, 0, sizeof(m_bKillCount));
		m_bUserStatus = 0;
	}
};
...
{
	CSTLMap < _CR_USER_LIST, std::string> m_bUserList;
	bool m_sCollectionStatus, m_bCollectionRequest;
	uint32 RewardItemID[3], RewardItemCount[3], RewardItemTime[3], m_bEventTime, m_bRankBug;
	uint8 RewardItemRate[3];
	uint8 RewardSession[3];
	uint16 m_bProtoID[3], m_bKillCount[3], m_bUserLimit, m_bTotalCount;
	uint8 m_bZoneID, m_bMinLevel, m_bMaxLevel, m_bCollectionEventListStatus;
	std::string m_bEventName;
	...
	void Initialize()
	{
		m_bUserList.DeleteAllData();
		m_sCollectionStatus = m_bCollectionRequest = false;
		m_bEventTime = 0;
		memset(&RewardItemID, 0, sizeof(RewardItemID));
		memset(&RewardItemCount, 0, sizeof(RewardItemCount));
		memset(&m_bProtoID, 0, sizeof(m_bProtoID));
		memset(&m_bKillCount, 0, sizeof(m_bKillCount));
		memset(&RewardItemRate, 0, sizeof(RewardItemRate));
		memset(&RewardItemTime, 0, sizeof(RewardItemTime));
		m_bUserLimit = m_bTotalCount = 0;
		m_bZoneID = m_bMinLevel = m_bMaxLevel = 0;
		m_bEventName.clear();
		m_bRankBug = 1;
		m_bCollectionEventListStatus = 0;
	}
};
...
{
	uint32_t RewardItemID[3], RewardItemCount[3], m_bEventTime, m_bEventID;
	uint16_t m_bProtoID[3], m_bKillCount[3], m_bUserLimit;
	uint8_t m_bZoneID, m_bMinLevel, m_bMaxLevel;
	std::string m_bEventName;
	uint8 m_bCollectionEventListStatus;
	bool m_bCollectionAutoStatus;
	int m_bCollectionAutoHour, m_bCollectionAutoMinute;
	...
	void Initialize()
	{
		m_bCollectionAutoStatus = false;
		m_bCollectionAutoHour = m_bCollectionAutoMinute = -1;
		m_bEventTime = 0;
		memset(&RewardItemID, 0, sizeof(RewardItemID));
		memset(&RewardItemCount, 0, sizeof(RewardItemCount));
		memset(&m_bProtoID, 0, sizeof(m_bProtoID));
...
struct TimerEventList
{
	...
	TimerEventList(std::string name, uint32 hour, uint32 minute) { this->name = name; this->hour = hour; this->minute = minute; }
};
