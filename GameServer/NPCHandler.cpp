...
void CUser::ItemRepair(Packet & pkt)
{
	...
	uint32 MoneyID, ItemID, sNpcID;
	uint16 durability, quantity = 0;
...
void CUser::ClientEvent(uint32 sNpcID)
{
	// Ensure AI's loaded
	if (isDead())
		return;
	...
	int32 iEventID = 0;
	CNpc *pNpc = g_pMain->GetNpcPtr((int16)sNpcID, GetZoneID());
	if (pNpc == nullptr
		|| !isInRange(pNpc, MAX_NPC_RANGE))
		return;
...
void CUser::ClassChange(Packet & pkt, bool bFromClient /*= true */)
{
	...
	else if (opcode == REB_STAT_RESET)
	{
		...
		foreach(itr, m_sPremiumMap)
		{
			...
			if (pPremium->bPremiumType == 12 
				|| pPremium->bPremiumType == 13)
				isHaveWARPre = true;
		}

		if (!hasCoins(100000000) 
			&& !isHaveWARPre)
			return;
	...
	if (isInParty())
	{
		...
		result << uint8(PARTY_CLASSCHANGE) << uint32(GetSocketID()) << uint16(classcode);
...
void CUser::SelectMsg(uint8 bFlag, int32 nQuestID, int32 menuHeaderText, int32 menuButtonText[MAX_MESSAGE_EVENT], int32 menuButtonEvents[MAX_MESSAGE_EVENT])
{
	...
	result << uint32(m_sEventSid) << bFlag << nQuestID << menuHeaderText;
...
void CUser::NpcEvent(Packet & pkt)
{
	...
	uint32 sNpcID = pkt.read<uint32>();
	int32 nQuestID = pkt.read<int32>();

	CNpc* pNpc = g_pMain->GetNpcPtr((uint16)sNpcID, GetZoneID());
	...
	case NPC_KJWAR:
		{
			...
			result << uint8(CAPURE_RIGHT_CLICK) << (uint32)GetSocketID() << GetName();
			Send(&result);
		}
		break;
#endif
...
void CUser::ItemTrade(Packet& pkt)
{
	...
	{
		pkt >> group >> npcid;
		int16 test = (int16)npcid;
		if ((pNpc = g_pMain->GetNpcPtr(test, GetZoneID())) == nullptr
	...
	if (type == 1 && pNpc->m_iSellingGroup != 249000)
	{
		for (int i = 0; i < purchased_item_count; i++) {
			...
			if (pItems[i].ITEMID != 0) {
				...
				auto m_sItemSellTableArray = g_pMain->m_ItemSellTableArray.m_UserTypeMap;
				g_pMain->m_ItemSellTableArray.m_lock.unlock();

				foreach(itr, m_sItemSellTableArray)
				{
					_ITEM_SELLTABLE* pSela = itr->second;
					if (pSela == nullptr 
						|| pSela->iSellingGroup != pNpc->m_iSellingGroup)
						continue;
		...
		for (int i = 0; i < purchased_item_count; i++)
		{
			...
			if (pItems[i].ITEMID != 0)
			{
				...
				foreach(itr, m_sItemSellTableArray)
				{
					...
					if (pSellt == nullptr 
						|| pSellt->iSellingGroup != pNpc->m_iSellingGroup)
						continue;
...
void CUser::SendRepurchaseMsg(bool isRefreshed /*= false*/)
{
	...
	m_sDeletedItemMap.m_lock.lock();
	auto m_bDeletedItemMap = m_sDeletedItemMap.m_UserTypeMap;
	m_sDeletedItemMap.m_lock.unlock();

	foreach(itr, m_bDeletedItemMap)
	{
		...
		nPrice = pItem.m_iBuyPrice * pItemDeleted->sCount * 30;
		if (nPrice > COIN_MAX)
			nPrice = COIN_MAX;
...
void CUser::BuyingItemRepurchase(Packet& pkt)
{
	...
	result << uint8(ProcDbType::ItemRemove) << uint8(1) << pItem->nSerialNum;
...
{
	m_sFreeRepurchaseIDLock.lock();
	if (m_sFreeRepurchaseID.size() <= 0)
	{
		printf("Max number of Item Remove. No ID available.\n");
		m_sFreeRepurchaseIDLock.unlock();
		return -1;
	}

	short RemoveID = *(m_sFreeRepurchaseID.begin());
	m_sFreeRepurchaseID.erase(m_sFreeRepurchaseID.begin());
	m_sFreeRepurchaseIDLock.unlock();
	return RemoveID;
}
...
{
	m_sFreeRepurchaseIDLock.lock();
	m_sFreeRepurchaseID.push_back(RemoveID);
