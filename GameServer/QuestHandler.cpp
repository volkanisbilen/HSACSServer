...
void CUser::QuestDataRequest(bool gamestarted)
{
	...
	foreach(itr, m_bQuestMap)
	{
		...
		{
#if(__VERSION < 2369)
			Packet test(WIZ_HSACS_HOOK, uint8(0xDF));
	...
	foreach(a, questlist)
	{
		_USER_QUEST_INFO* pInfo = m_sQuestMap.GetData(*a);
		if (pInfo == nullptr)
			continue;
...
QuestClientPacket:
	{
		...
		case 5:
		{
			_USER_QUEST_INFO * pQuestInfo = m_sQuestMap.GetData(pQuestHelper->sEventDataIndex);
...
void CUser::SaveEvent(uint16 sQuestID, uint8 bQuestState)
{
	...
	case 1:
	{
		_USER_QUEST_INFO* pQuestInfo = m_sQuestMap.GetData(sQuestID);
		if (pQuestInfo == nullptr)
		{
			...
			if (!m_sQuestMap.PutData(sQuestID, pUserQuest))
			{
				delete pUserQuest;
				return;
			}
...
{
	m_sQuestMap.DeleteData(sQuestID);
}
...
{
	// Attempt to find a quest with that ID in the map
	_USER_QUEST_INFO* pQuestInfo = m_sQuestMap.GetData(QuestID);
