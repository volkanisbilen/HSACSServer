...
{
	Sleep(1 * SECOND);
	m_ZoneArray.m_lock.lock();
	auto m_sZoneArray = m_ZoneArray.m_UserTypeMap;
	m_ZoneArray.m_lock.unlock();

	foreach(itr, m_sZoneArray)
	{
...
bool CGameServerDlg::LoadBotInfoTable(bool m_bStatus)
{
	if (m_bStatus == false) { LOAD_TABLE_ERROR_ONLY(CBotMerchantInfoSet, g_DBAgent.m_GameDB, &pBotInfo, true, false); }
...
bool CGameServerDlg::LoadUserRankings()
{
	...
	CBotPersonalRankSet BotPersonalRankSet(g_DBAgent.m_GameDB, &m_BotKarusPersonalRankMap, &m_BotElmoPersonalRankMap);
	CUserKnightsRankSet  UserKnightsRankSet(g_DBAgent.m_GameDB, &m_UserKarusKnightsRankMap, &m_UserElmoKnightsRankMap);
	CBotKnightsRankSet  BotKnightsRankSet(g_DBAgent.m_GameDB, &m_BotKarusKnightsRankMap, &m_BotElmoKnightsRankMap);
	...
	szError = BotPersonalRankSet.Read(true);
	if (szError != nullptr)
	{
		printf("ERROR: Failed to load personal rankings, error:\n%s\n", szError);
		return false;
	...
	szError = BotKnightsRankSet.Read(true);
	if (szError != nullptr)
	{
		printf("ERROR: Failed to load user knights rankings, error:\n%s\n", szError);
		return false;
	...
	std::vector< _USER_RANK> m_botpersokarus, m_botpersoelmorad, m_botknightkarus, m_botknightelmorad;
	...
	foreach(itr, m_BotKarusPersonalRankMap)
		if (itr->second) m_botpersokarus.push_back(*itr->second);

	foreach(itr, m_BotElmoPersonalRankMap)
		if (itr->second) m_botpersoelmorad.push_back(*itr->second);

	foreach(itr, m_BotKarusKnightsRankMap)
		if (itr->second) m_botknightkarus.push_back(*itr->second);

	foreach(itr, m_BotElmoKnightsRankMap)
		if (itr->second) m_botknightelmorad.push_back(*itr->second);
	...
	foreach(itr, m_botpersokarus)
	{
		CBot* pUser = GetBotPtr(itr->strUserName, NameType::TYPE_CHARACTER);
		if (pUser)
			pUser->m_bPersonalRank = uint8(itr->nRank);
	}

	foreach(itr, m_botpersoelmorad)
	{
		CBot* pUser = GetBotPtr(itr->strUserName, NameType::TYPE_CHARACTER);
		if (pUser)
			pUser->m_bPersonalRank = uint8(itr->nRank);
	...
	foreach(itr, m_botknightelmorad)
	{
		CBot* pUser = GetBotPtr(itr->strUserName, NameType::TYPE_CHARACTER);
		if (pUser)
			pUser->m_bKnightsRank = uint8(itr->nRank);
...
bool CGameServerDlg::LoadRightClickExchangeTable() { return g_DBAgent.LoadRightClickExchangeTable(); }
bool CGameServerDlg::LoadRightExchangeTable() { return g_DBAgent.LoadRightExchangeTable(); }
#endif
...
void CGameServerDlg::CleanupBotRankings()
{
	std::set<_USER_RANK*> deleteSet, deleteSet2, deleteSet3, deleteSet4;
	std::vector< std::string> m_prank, m_krank;
	...
	m_BotElmoPersonalRankMap.clear();
	m_BotKarusPersonalRankMap.clear();
	m_BotElmoKnightsRankMap.clear();
	m_BotKarusKnightsRankMap.clear();
	...
	foreach(itr, m_prank)
	{
		CBot* pUser = GetBotPtr(*itr, NameType::TYPE_CHARACTER);
		if (pUser != nullptr) pUser->m_bPersonalRank = -1;
	}
...
bool CGameServerDlg::LoadPusCategoryTable() { LOAD_TABLE(CPusItemCategory, g_DBAgent.m_GameDB, &m_PusCategoryArray, true, false); }

bool CGameServerDlg::LoadObjectPosTable() { LOAD_TABLE(CObjectPosSet, g_DBAgent.m_GameDB, &m_ObjectEventArray, true, false); }

bool CGameServerDlg::LoadSaveBotDataTable() { LOAD_TABLE(CBotSaveDataSet, g_DBAgent.m_GameDB, &m_bBotSaveDataArray, true, false); }

bool CGameServerDlg::LoadPusItemsTable() { LOAD_TABLE(CPusItemSet, g_DBAgent.m_GameDB, &m_PusItemArray, false, false); }

bool CGameServerDlg::LoadTimedNoticeTable() { LOAD_TABLE(CTimedNotice, g_DBAgent.m_GameDB, &m_TimedNoticeArray, true, false); }

bool CGameServerDlg::LoadRankBugTable() { LOAD_TABLE(CRankBugSet, g_DBAgent.m_GameDB, nullptr, true, false); }

bool CGameServerDlg::LoadUserLootSettingsTable() { LOAD_TABLE(CUserLootSettingsSet, g_DBAgent.m_GameDB, &UserLootSettings, false, false); }
...
bool CGameServerDlg::LoadBotMerchantTable() { return g_DBAgent.LoadBotHandlerMerchantTable(); }