...
void CUser::SelectCharacter(Packet & pkt)
{
	...
	if ((pUser && pUser->m_bSelectedCharacter))
		return goDisconnect("HSACSXSoft - Dominik | DC [1].", __FUNCTION__);
	...
	case 1:
	{
		...
		if (!ishome && GetZoneID() == ZONE_ELMORAD && !g_pMain->m_byElmoradOpenFlag && GetNation() == (uint8)Nation::KARUS) { ishome = true; zoneid = ZONE_MORADON; }
		...
		if (!ishome && g_pMain->pSpecialEvent.opened && isInPKZone()) { ishome = true; zoneid = ZONE_MORADON; }
		if (!ishome && (isPartyTournamentinZone() || isClanTournamentinZone())) { auto* TournamentClanInfo = g_pMain->m_ClanVsDataList.GetData(GetZoneID());			if ((TournamentClanInfo == nullptr) || (TournamentClanInfo->aTournamentClanNum[0] != GetClanID() && TournamentClanInfo->aTournamentClanNum[1] != GetClanID())) { ishome = true; zoneid = ZONE_MORADON; } }
		if (!ishome && GetZoneID() == ZONE_BIFROST) { if (!g_pMain->pBeefEvent.isActive || !g_pMain->pBeefEvent.isMonumentDead) { ishome = true; zoneid = ZONE_MORADON; }if (g_pMain->pBeefEvent.isActive && !g_pMain->pBeefEvent.isFarmingPlay) { ishome = true; zoneid = ZONE_MORADON; }if (g_pMain->pBeefEvent.isActive && g_pMain->pBeefEvent.isFarmingPlay && g_pMain->pBeefEvent.WictoryNation != GetNation() && !g_pMain->pBeefEvent.isLoserSign) { ishome = true; zoneid = ZONE_MORADON; } }
		if (!ishome && g_pMain->isCindirellaZone(GetZoneID())) { ishome = true; zoneid = ZONE_MORADON; }
		if (!ishome && GetZoneID() == ZONE_DELOS && !CastleSiegeWarfareCanenterDelos()) { ishome = true; zoneid = ZONE_MORADON; }
		...
		result << bResult << GetZoneID() << GetSPosX() << GetSPosZ() << GetSPosY();
#else
		result << bResult << uint16(GetZoneID()) << GetSPosX() << GetSPosZ() << GetSPosY() << GetNation() << int16(-1);
#endif
	...
	case 4:
	{
		...
		if (strCharID1.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID1, ZoneID))
			{
				...
				{
					result << uint8(PrisonCharacter) << strCharID1 << "HSACSXSoft - Dominik";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID2.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID2, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID2 << "HSACSXSoft - Dominik";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID3.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID3, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID3 << "HSACSXSoft - Dominik";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID4.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID4, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID4 << "HSACSXSoft - Dominik";
					Send(&result);
					return;
				}
			}
...
void CUser::SendServerChange(std::string & ip, uint8 bInit)
{
	...
	result << ip << uint16(g_pMain->m_GameServerPort) << bInit << uint16(GetZoneID()) << GetNation();
	Send(&result);
}
...
void CUser::SendLists() {
	...
	HandleHShieldSoftwareRightExchangeLoadderHandler();
	HandleHShieldSoftwareDailyReward();
#else
	HandleHShieldSoftwareRightExchangeLoadderHandler();
	HandleHShieldSoftwareDailyReward();
#endif
...
void CUser::SendEventTimerList() {
	...
	Packet newpkt(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::EventShowList));

	g_pMain->m_sTimeShowEventListLock.lock();
	newpkt << nHour << nMinute << nSeconds << (uint16)g_pMain->m_sTimeShowEventList.size();
	foreach(itr, g_pMain->m_sTimeShowEventList) newpkt << itr->name << itr->hour << itr->minute;
	g_pMain->m_sTimeShowEventListLock.unlock();
	Send(&newpkt);
#endif
}
...
void CUser::UserInfoStorage(bool login) {
	...
	if (login) {
		if (itr->second.flamebacktime > UNIXTIME)
			m_bFlamelevel = itr->second.flamelevel;
		return;
	}

	itr->second.flamelevel = m_bFlamelevel;
...
void CUser::GameStart(Packet & pkt)
{
	...
	else if (opcode == 2)
	{
		...
		std::vector< _SEND_MESSAGE> m_sMessageList;
		g_pMain->m_SendMessageArray.m_lock.lock();
		auto m_sSendMessageArray = g_pMain->m_SendMessageArray.m_UserTypeMap;
		g_pMain->m_SendMessageArray.m_lock.unlock();

		foreach(itr, m_sSendMessageArray)
		{
			...
			uint8 type = (uint8)msg->bChatType;
			m_sMessageList.push_back(*itr->second);
		}

		foreach(itr, m_sMessageList)
		{
			Packet result;
...
{
#if(__VERSION < 2369)
	HSACSX_UIRequest(Packet(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::UIINFO)));
#endif
}
#pragma endregion
...
void CUser::SendAntiAfkList() {
	...
	uint16 counter = 0;
	Packet test(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::SendAntiAfk));
	test << counter;
