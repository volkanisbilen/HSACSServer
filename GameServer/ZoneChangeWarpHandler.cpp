...
#else
	result << uint16(GetZoneID()) << uint16(0) << GetSPosX() << GetSPosZ() << GetSPosY() << GetNation() << uint16(-1); /*<< GetSPosY() << GetSPosX() << GetSPosZ() << GetNation() << g_pMain->m_byOldVictory;*/
#endif
	//printf("WIZ_ZONE_CHANGE %d %d %d\n", GetSPosX(), GetSPosY(), GetSPosZ());
	Send(&result);
...
void CUser::RecvZoneChange(Packet & pkt)
{
	...
	uint16 opcodes = pkt.read<uint16>();
	//printf("RecvZoneChange %d %d\n", opcode, opcodes);
