...
void CKnightsManager::PacketProcess(CUser *pUser, Packet & pkt)
{
	...
	//printf("Clan packet system opcode: %X\n", opcode);
	...
	break;
	case KnightsPacket::KNIGHTS_PROMATE_CLAN:
		break;
	default:
...
void CKnightsManager::KnightsFlagsList(CUser* pUser, Packet& pkt)
{
	...
	Packet result(WIZ_KNIGHTS_PROCESS, uint8(KnightsPacket::KNIGHTS_UNK1));
	result << uint16(0x01);
	...
	for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++)
	{
		...
		auto m_sKnightsRatingArray = g_pMain->m_KnightsRatingArray[nation].m_UserTypeMap;
		g_pMain->m_KnightsRatingArray[nation].m_lock.unlock();

		foreach(itr, m_sKnightsRatingArray)
		{
...
void CKnightsManager::KnightsJoin(CUser* pUser, Packet& pkt)
{
	...
	uint32 TargetID = pkt.read<uint32>();
	int16 sTargetID = (int16)TargetID;
	...
	result << uint8(KnightsPacket::KNIGHTS_JOIN_REQ) << uint8(1)
		<< uint32(pUser->GetSocketID()) << pUser->GetClanID() << pKnights->m_strName;
	pTUser->Send(&result);
...
void CKnightsManager::KnightsJoinReq(CUser *pUser, Packet & pkt)
{
	...
	int16 m_SocketID = (int16)sid;
	CUser *pTUser = g_pMain->GetUserPtr(m_SocketID);
	if (pTUser == nullptr)
		bResult = 2;
	else if (bFlag == 0)
		bResult = 11;
...
void CKnightsManager::KnightsAllMember(CUser *pUser)
{
	...
	if (bResult == 1)
	{
		...
		result << pktSize << uint16(g_DBAgent.KnightsCount(pKnights->GetID())) << uint16(MAX_CLAN_USERS) << pKnights->m_strClanNotice;
...
void CKnightsManager::KnightsCurrentMember(CUser *pUser, Packet & pkt)
{
	...
	auto m_sKnightUser = pKnights->m_arKnightsUser.m_UserTypeMap;
	pKnights->m_arKnightsUser.m_lock.unlock();

	foreach(itr, m_sKnightUser)
		if (itr->second) m_userlist.push_back(itr->second->strUserName);
...
void CKnightsManager::KnightsAllList(CUser *pUser, Packet & pkt)
{
	...
	auto m_sKnightArray = g_pMain->m_KnightsArray.m_UserTypeMap;
	g_pMain->m_KnightsArray.m_lock.unlock();

	foreach(itr, m_sKnightArray)
	{
...
void CKnightsManager::KnightsGetSymbol(CUser* pUser, uint16 sClanID)
{
	...
	result	<< uint8(KnightsPacket::KNIGHTS_MARK_REQ) 
		<< uint16(1) // success
		<< uint16(pKnights->m_byNation) 
		<< sClanID
		<< uint16(pKnights->m_sMarkVersion) 
		<< uint16(pKnights->m_sMarkLen);
...
int CKnightsManager::GetKnightsIndex(int nation)
{
	...
	if ((nation == (uint8)Nation::KARUS && knightindex >= 15000) 
		|| (nation == (uint8)Nation::ELMORAD && knightindex < 15000))
		return -1;
...
void CKnightsManager::DonationList(CUser *pUser, Packet & pkt)
{
	...
	pKnights->m_arKnightsUser.m_lock.lock();
	auto m_sUserKnightList = pKnights->m_arKnightsUser.m_UserTypeMap;
	pKnights->m_arKnightsUser.m_lock.unlock();

	foreach(pKnightList, m_sUserKnightList)
	{
		_KNIGHTS_USER * p = pKnightList->second;
		if(p == nullptr)
			continue;
...
void CKnightsManager::KnightsLadderPointList(CUser* pUser, Packet& pkt)
{
	...
	if (pKnights->m_sLadderPointTime > UNIXTIME) 
	{
		if (pKnights->m_sLadderPointList.empty())
			goto show_reset;

		foreach(list, pKnights->m_sLadderPointList)
		{ 
			result << list->name << list->loyalty; 
			count++; 
		}
...
show_reset:
	pKnights->m_sLadderPointTime = UNIXTIME + 60;
	pKnights->m_sLadderPointList.clear();

	pKnights->m_arKnightsUser.m_lock.lock();
	auto m_sUserKnightList = pKnights->m_arKnightsUser.m_UserTypeMap;
	pKnights->m_arKnightsUser.m_lock.unlock();

	foreach(pKnightUser, m_sUserKnightList)
	{ 
		auto* pKnightList = pKnightUser->second;
		if (pKnightList) 
			pKnights->m_sLadderPointList.push_back(LadderPointList(pKnightList->strUserName, pKnightList->LoyaltyMonthly));
	}

	if (pKnights->m_sLadderPointList.empty())
		return;

	std::sort(pKnights->m_sLadderPointList.begin(), pKnights->m_sLadderPointList.end(), [](auto const& a, auto const& b) { return a.loyalty > b.loyalty; });
	foreach(list, pKnights->m_sLadderPointList)
	{ 
		result << list->name << list->loyalty; 
		count++; 
