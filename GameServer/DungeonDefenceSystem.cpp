...
void CGameServerDlg::DungeonDefenceUserisOut(uint16 EventRoom)
{
	...
	auto m_UserList = pRoomInfo->m_UserList.m_UserTypeMap;
	pRoomInfo->m_UserList.m_lock.unlock();

	foreach(itrUser, m_UserList)
	{
...
uint8 CGameServerDlg::DungeonDefenceSelectStage(uint16 EventRoom)
{
	...
	m_DungeonDefenceStageListArray.m_lock.lock();
	auto m_sDungeonDefenceStageListArray = m_DungeonDefenceStageListArray.m_UserTypeMap;
	m_DungeonDefenceStageListArray.m_lock.unlock();

	foreach(itr, m_sDungeonDefenceStageListArray)
	{
...
void CGameServerDlg::SummonDungeonDefenceMonsters(uint16 EventRoom)
{
	...
	m_DungeonDefenceMonsterListArray.m_lock.lock();
	auto m_sDungeonDefenceMonsterListArray = m_DungeonDefenceMonsterListArray.m_UserTypeMap;
	m_DungeonDefenceMonsterListArray.m_lock.unlock();

	foreach(itr, m_sDungeonDefenceMonsterListArray)
	{
...
void CUser::DungeonDefenceSendFinishTimer()
{
	...
	result << uint32(0) << uint8(7) << uint64(0) << uint8(9) << uint16(0) << uint8(0) << uint8(11) << uint16(pRoomInfo->m_DefenceOutTime) << uint16(0);
	g_pMain->Send_Zone(&result, ZONE_DUNGEON_DEFENCE, nullptr, (uint8)Nation::ALL, GetEventRoom());
}
#pragma endregion
