...
COMMAND_HANDLER(CUser::HandleReloadTable)
{
	...
	else if (strTable == "MAGIC")
	{
		...
		g_pMain->m_Magictype7Array.DeleteAllData();
	...
	else
	{
		...
		else
		{
			...
			else if (strTable == "BOT_HANDLER_FARM")
			{
				...
				g_pMain->m_sBotCharackterNameArray.clear();
				g_pMain->m_sBotCharackterNameLock.unlock();
				g_pMain->m_sMapBotListArray.DeleteAllData();
			...
			else if (strTable == "ITEM_RIGHT_CLICK_EXCHANGE")
			{
#if(GAME_SOURCE_VERSION == 2369)
				g_pMain->s_HShieldSoftwareRightClickExchangeArray.DeleteAllData();
				g_pMain->LoadRightClickExchangeTable();
				...
				if (s_HShieldSoftwareRightClickExchangeArray.size() > 0)
				{
					for (uint16 i = 0; i < MAX_USER; i++)
					{
						...
						pUser->HandleHShieldSoftwareNewItemMoveLoadderHandler();
					}
				}
#endif
				...
				g_pMain->LoadRightClickExchangeTable();
#endif
				g_pMain->s_HShieldSoftwareRightExchangeArray.DeleteAllData();
				g_pMain->LoadRightExchangeTable();
				...
				if (s_HShieldSoftwareRightExchangeArray.size() > 0)
				{
					for (uint16 i = 0; i < MAX_USER; i++)
					{
						CUser* pUser = g_pMain->GetUserPtr(i);
						...
						pUser->HandleHShieldSoftwareRightExchangeLoadderHandler();
					}
				}
				isResult = true;
			}
...
COMMAND_HANDLER(CUser::HandleMonsterSummonCommand)
{
	...
	g_pMain->m_sSummonMessage = true;
	g_pMain->m_sSummonGameMaster = GetName().c_str();
	return true;
}
#pragma endregion
...
void CGameServerDlg::LotteryClose(CUser* pUser /*=nullptr*/)
{
	...
	if (!m_bRimaLotteryUserInfo.empty())
	{
	...
	bool m_bLotteryStatus = true;
	if (mList.empty())
		m_bLotteryStatus = false;
...
COMMAND_HANDLER(CUser::HandleJobChangeGM)
{
	...
	{
		// send description
		g_pMain->SendHelpDescription(this, "Using Sample : +jobchange nickname 1/2/3/4/5 ");
		...
		g_pMain->SendHelpDescription(this, "jobchange nickname 5 Kurian");
		return true;
	}
...
COMMAND_HANDLER(CUser::HandleKcChangeCommand) // gm koduyla kc yukleme veya eksiltme
{
	...
	if (KC != 0)
	{
		...
		result << uint8(HSACSXOpCodes::KCUPDATE) << uint32(KC) << pUser->m_nTLBalance;
		pUser->Send(&result);
#endif
	}
...
COMMAND_HANDLER(CUser::HandleTLBalanceCommand) //16.09.2020 gm koduyla kc yukleme veya eksiltme
{
	...
	if (KC != 0)
	{
		...
		Packet result(WIZ_HSACS_HOOK);
		result << uint8(HSACSXOpCodes::KCUPDATE) << uint32(pUser->m_nKnightCash) << pUser->m_nTLBalance;
		pUser->Send(&result);*/
	}
...
COMMAND_HANDLER(CUser::HandleModeGameMaster)
{
	if (!isGM())
		return false;
#if(__VERSION < 2369)
#if(GAME_SOURCE_VERSION == 1098)
	Packet result(WIZ_HSACS_HOOK);
	result << uint8(HSACSXOpCodes::WIZ_GAME_MASTER_MODE);
	Send(&result);
#endif
#endif
	return true;
