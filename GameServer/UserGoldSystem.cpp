...
void CUser::GoldGain(uint32 gold, bool bSendPacket /*= true*/, bool bApplyBonus /*= false*/)
{
	...
	{ 
		...
		if (m_bFlamelevel > 0 && m_bFlamelevel <= 3 && g_pMain->pBurningFea[m_bFlamelevel - 1].moneyrate)
			gold = gold * (100 + g_pMain->pBurningFea[m_bFlamelevel - 1].moneyrate) / 100;
...
bool CUser::CashLose(uint32 cash)
{
	...
	result << uint8(HSACSXOpCodes::CASHCHANGE) << m_nKnightCash <<  m_nTLBalance;
	Send(&result);
#endif
