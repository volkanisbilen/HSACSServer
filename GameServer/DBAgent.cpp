...
void CDBAgent::LoadCharInfo(string& strCharID, ByteBuffer& result)
{
	...
	result << strCharID << bRace << sClass << (int16)bLevel /* << (int16)bFace*/ << nHair << (int16)bZone;
...
bool CDBAgent::LoadUserData(string& strAccountID, string& strCharID, CUser* pUser)
{
	...
	for (int i = 0; i < INVENTORY_TOTAL; i++)
	{
		...
		if (i == SHOULDER
			...
			pUser->m_bAutoLoot = true;

		if (i == CFAIRY && nItemID == ITEM_OREADS)
			pUser->m_bFairyCheck = true;
		...
		if (pUser->dupplicate_itemcheck(pItem->nSerialNum))
		{
			...
			if (!pTable.isnull())
			{
				TRACE("Dupplicate Item Inventory: AccountName:%s - UserName:%s -  [Item Name:%s, ItemNum:%d, SerialNum:%I64d, ItemSlot:%d] \n",
...
bool CDBAgent::LoadWarehouseData(string& strAccountID, CUser* pUser)
{
	...
	for (int i = 0; i < WAREHOUSE_MAX; i++)
	{
		...
		if (pUser->dupplicate_itemcheck(pUser->m_sWarehouseArray[i].nSerialNum))
		{
			...
			if (!pTable.isnull())
			{
				TRACE("Dupplicate Item Warehouse: AccountName:%s - UserName:%s -  [Item Name:%s, ItemNum:%d, SerialNum:%I64d, ItemSlot:%d] \n",
...
bool CDBAgent::LoadVipWarehouseData(string& strAccountID, CUser* pUser)
{
	...
	for (int i = 0; i < VIPWAREHOUSE_MAX; i++)
	{
		...
		if (pUser->dupplicate_itemcheck(pUser->m_sVIPWarehouseArray[i].nSerialNum))
		{
			...
			if (!pTable.isnull())
			{
				TRACE("Dupplicate Item WipWarehouse: AccountName:%s - UserName:%s -  [Item Name:%s, ItemNum:%d, SerialNum:%I64d, ItemSlot:%d] \n",
...
bool CDBAgent::LoadPremiumServiceUser(string& strAccountID, CUser* pUser)
{
	...
	if (pUser->m_PremiumMap.GetData(pUser->m_bPremiumInUse) == nullptr)
	{
		...
		pUser->m_PremiumMap.m_lock.lock();
		auto m_sPremiumMap = pUser->m_PremiumMap.m_UserTypeMap;
		pUser->m_PremiumMap.m_lock.unlock();
		...
		if (pKingSystem != nullptr)
		{
			if (dbCommand->hasData())
			{
				...
				if (pUser != nullptr && pUser->isInGame())
				{
					...
					pUser->RegionNpcInfoForMe();
					pUser->RegionUserInOutForMe();
					pUser->MerchantUserInOutForMe();
					pUser->ResetWindows();
					pUser->InitType4();
...
void CDBAgent::LoadUpgrade()
{
	...
	g_pMain->m_sLoadUpgradeArray.clear();
	...
	if (dbCommand->hasData())
	{
		do
		{
			...
			g_pMain->m_sLoadUpgradeArray.insert(std::make_pair(nIndex, pEvent));
			rCount++;
...
void CDBAgent::LoadItemUpgradeSettings()
{
	...
	g_pMain->m_sUpgradeSettingArray.clear();
	uint32 rCount = 1;
	...
	if (dbCommand->hasData())
	{
		do
		{
			...
			g_pMain->m_sUpgradeSettingArray.insert(std::make_pair(rCount, pUpgrade));
			rCount++;
...
bool CDBAgent::LoadAllKnights(KnightsArray& m_KnightsArray)
{
	...
	do
	{
		...
		if (pData->m_sCastTime != 0 && (uint32)UNIXTIME < pData->m_sCastTime)pData->m_sCastellanCape = true;
...
#pragma region CDBAgent::KnightsCount(uint16 sClanID)
int8 CDBAgent::KnightsCount(uint16 sClanID)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return 0;

	if (!dbCommand->Execute(string_format(_T("SELECT * FROM USER_KNIGHTDATA WHERE sClanID = %d"), sClanID)))
		ReportSQLError(m_GameDB->GetError());
	...
	uint8 count = 0;
	do { count++; } while (dbCommand->MoveNext());
...
bool CDBAgent::UpdateAchieveData(CUser* pUser)
{
	...
	*(uint16*)(strAchieve + 34) = pUser->m_AchieveInfo.ChallangeCount;
	...
	pUser->m_AchieveMap.m_lock.lock();
	auto m_sAchieveMap = pUser->m_AchieveMap.m_UserTypeMap;
	pUser->m_AchieveMap.m_lock.unlock();

	foreach(itr, m_sAchieveMap)
	{
		if (itr->second == nullptr)
			continue;
...
bool CDBAgent::LoadAchieveData(CUser* pUser)
{
	...
	{
		g_pMain->m_AchieveMainArray.m_lock.lock();
		auto m_sAchieveMainArray = g_pMain->m_AchieveMainArray.m_UserTypeMap;
		g_pMain->m_AchieveMainArray.m_lock.unlock();

		foreach(itr, m_sAchieveMainArray)
		{
...
{
	g_pMain->m_CharacterSealItemArray.m_lock.lock();
	auto m_sCharacterSealItemArray = g_pMain->m_CharacterSealItemArray.m_UserTypeMap;
	g_pMain->m_CharacterSealItemArray.m_lock.unlock();

	foreach(itr, m_sCharacterSealItemArray)
	{
...
void CDBAgent::LoadPetData(uint64 nSerial)
{
	...
	if (pInfo == nullptr)
	{
		...
		if (pInfo == nullptr)
		{
			...
			delete p; // Remember to delete p to avoid memory leak
			return;
		}
	}
	...
	// Deep copy the PET_INFO data
	p->info = new PET_INFO(*pInfo);
...
bool CDBAgent::LoadBotTable()
{
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_BOT_HANDLER}")))
	{
		printf(" Error LOAD_BOT_HANDLER. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
...
uint8 CDBAgent::LoadPPCard(std::string PPCardKeys, CUser* pUser)
{
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, PPCardKeys.c_str(), PPCardKeys.length());
	if (!dbCommand->Execute(string_format(_T("UPDATE PPCARD_LIST SET [STATUS] = 1 , strAccountID = '%s' , strUserID = '%s' , UpdateDate = convert(varchar, getdate(), 120) FROM PPCARD_LIST WHERE PPKeyCode = ? AND [STATUS] = 0"),	pUser->GetAccountName().c_str(), pUser->GetName().c_str()))){
		ReportSQLError(m_GameDB->GetError());
		return 0;
	}
	pUser->GiveBalance(Cash, tlcount);
...
bool CDBAgent::LoadRightClickExchangeTable()
{
	std::unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("SELECT * FROM ITEM_RIGHT_CLICK_EXCHANGE"))) {
		printf(" Error ITEM_RIGHT_CLICK_EXCHANGE.\n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	{
		int i = 1;
		_ITEM_RIGHT_CLICK_EXCHANGE* pData = new _ITEM_RIGHT_CLICK_EXCHANGE;

		dbCommand->FetchUInt32(i++, pData->sItemID);
		dbCommand->FetchByte(i++, pData->sSobCode);

		if (!g_pMain->s_HShieldSoftwareRightClickExchangeArray.PutData(pData->sItemID, pData))
			delete pData;

	} while (dbCommand->MoveNext());
	return true;
}
bool CDBAgent::LoadRightExchangeTable()
{
	std::unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("SELECT * FROM ITEM_RIGHT_EXCHANGE"))) {
		printf(" Error ITEM_RIGHT_EXCHANGE.\n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	{
		int i = 1;
		_ITEM_RIGHT_EXCHANGE* pData = new _ITEM_RIGHT_EXCHANGE;

		dbCommand->FetchUInt32(i++, pData->sItemID);
		dbCommand->FetchString(i++, pData->strName);
		dbCommand->FetchByte(i++, pData->sType);
		dbCommand->FetchString(i++, pData->Description);
		dbCommand->FetchUInt32(i++, pData->sCount);

		for (int x = 0; x < ITEMS_RIGHT_CLICK_EXCHANGE_GROUP; x++)
		{
			dbCommand->FetchUInt32(i++, pData->nItemID[x]);
			dbCommand->FetchUInt32(i++, pData->nCount[x]);
			dbCommand->FetchUInt32(i++, pData->nExpirationTime[x]);
		}
		if (!g_pMain->s_HShieldSoftwareRightExchangeArray.PutData(pData->sItemID, pData))
			delete pData;
...
uint8 CDBAgent::LoadDailyReward(std::vector<ByteBuffer>& sItemList)
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;

	if (!dbCommand->Execute(_T("SELECT * FROM DAILY_REWARD")))
	{
		ReportSQLError(m_AccountDB->GetError());
		return false;
	}
	...
	if (dbCommand->hasData())
	{
		do
		{
			dbCommand->FetchUInt32(1, sItemID[0]);
			dbCommand->FetchUInt32(2, sItemID[1]);
			dbCommand->FetchUInt32(3, sItemID[2]);
			dbCommand->FetchUInt32(4, sItemID[3]);
			dbCommand->FetchUInt32(5, sItemID[4]);
			dbCommand->FetchUInt32(6, sItemID[5]);
			dbCommand->FetchUInt32(7, sItemID[6]);
			dbCommand->FetchUInt32(8, sItemID[7]);
			dbCommand->FetchUInt32(9, sItemID[8]);
			dbCommand->FetchUInt32(10, sItemID[9]);
			dbCommand->FetchUInt32(11, sItemID[10]);
			dbCommand->FetchUInt32(12, sItemID[11]);
			dbCommand->FetchUInt32(13, sItemID[12]);
			dbCommand->FetchUInt32(14, sItemID[13]);
			dbCommand->FetchUInt32(15, sItemID[14]);
			dbCommand->FetchUInt32(16, sItemID[15]);
			dbCommand->FetchUInt32(17, sItemID[16]);
			dbCommand->FetchUInt32(18, sItemID[17]);
			dbCommand->FetchUInt32(19, sItemID[18]);
			dbCommand->FetchUInt32(20, sItemID[19]);
			dbCommand->FetchUInt32(21, sItemID[20]);
			dbCommand->FetchUInt32(22, sItemID[21]);
			dbCommand->FetchUInt32(23, sItemID[22]);
			dbCommand->FetchUInt32(24, sItemID[23]);
			dbCommand->FetchUInt32(25, sItemID[24]);

		} while (dbCommand->MoveNext());
	}

	ByteBuffer sItemBuffer;
	for (int i = 0; i < 25; i++)
		sItemBuffer << sItemID[i];
	sItemList.push_back(sItemBuffer);
	return 0;
}

uint8 CDBAgent::LoadDailyCumulativeReward(std::vector<ByteBuffer>& sItemList)
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;

	if (!dbCommand->Execute(_T("SELECT * FROM DAILY_REWARD_CUMULATIVE")))
	{
		ReportSQLError(m_AccountDB->GetError());
		return false;
	}
...
uint8 CDBAgent::LoadDailyRewardUser(uint16 UserID, std::vector<ByteBuffer>& sItemList)
{
	unique_ptr<OdbcCommand> dbCommand(m_AccountDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;
	...
	if (!dbCommand->Execute(string_format(_T("SELECT strDateTime FROM DAILY_REWARD_USER WHERE strUserID = '%s' "), pUser->GetName().c_str())))
	{
		ByteBuffer sDateTime;
		for (int i = 0; i < 25; i++)
			sDateTime << uint8(0) << uint8(0)/*day */;

		dbCommand->AddParameter(SQL_PARAM_INPUT, (char*)sDateTime.contents(), sDateTime.size(), SQL_BINARY);
		if (!dbCommand->Execute(string_format(_T("INSERT INTO DAILY_REWARD_USER (strDateTime, strUserID) VALUES(?, '%s') "), pUser->GetName().c_str())))
		{
			ReportSQLError(m_AccountDB->GetError());
			return false;
		}
	...
	if (dbCommand->hasData())
	{
		...
		{
			char sDateTime[50] = { 0 };
			dbCommand->FetchBinary(1, sDateTime, sizeof(sDateTime));
			ByteBuffer sDateTimeBuffer;
			sDateTimeBuffer.append(sDateTime, sizeof(sDateTime));
			sItemList.push_back(sDateTimeBuffer);
...
uint8 CDBAgent::UpdateDailyRewardUser(uint16 UserID, uint8 sData[], uint8 sDatatime[])
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return 0;
	...
	ByteBuffer sDataTime;
	for (int i = 0; i < 25; i++)
		sDataTime << uint8(0) << uint8(0)/*day */;

	ByteBuffer sRealTime;
	for (int i = 0; i < 25; i++)
		sRealTime << uint8(sData[i]) << uint8(sDatatime[i])/*day */;

	if (!dbCommand->Execute(string_format(_T("SELECT strDateTime FROM DAILY_REWARD_USER WHERE strUserID = '%s' "), pUser->GetName().c_str())))
	{
		dbCommand->AddParameter(SQL_PARAM_INPUT, (char*)sDataTime.contents(), sDataTime.size(), SQL_BINARY);
		if (!dbCommand->Execute(string_format(_T("INSERT INTO DAILY_REWARD_USER (strDateTime, strUserID) VALUES(?, '%s') "), pUser->GetName().c_str())))
		{
			ReportSQLError(m_AccountDB->GetError());
			return false;
		...
		dbCommand->AddParameter(SQL_PARAM_INPUT, (char*)sRealTime.contents(), sRealTime.size(), SQL_BINARY);
		if (!dbCommand->Execute(string_format(_T("UPDATE DAILY_REWARD_USER	SET strDateTime = ? where strUserID = '%s'"), pUser->GetName().c_str())))
		{
			ReportSQLError(m_AccountDB->GetError());
			return false;
		}
