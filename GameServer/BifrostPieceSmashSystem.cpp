...
{
	uint16 sNpcID; uint32 nExchangeItemID, m_sNpcID; int8 bSrcPos = -1, errorcode = 2;
	pkt >> m_sNpcID >> nExchangeItemID >> bSrcPos;
	printf("BifrostPieceProcess %d %d %d\n", m_sNpcID, nExchangeItemID, bSrcPos);
	Packet result(WIZ_ITEM_UPGRADE, (uint8)ItemUpgradeOpcodes::ITEM_BIFROST_EXCHANGE);
	...
	Send(&result);
	{
		...
		newpkt << (uint8)beefEffectType << uint32(GetTargetID());
