...
void CGameServerDlg::ReqBotLoadSaveData(Packet& pkt) {
	...
	{
		g_pMain->m_bBotSaveDataArray.DeleteAllData();
		...
		g_pMain->m_bBotSaveDataArray.m_lock.lock();
		auto m_bBotSaveDataArray = g_pMain->m_bBotSaveDataArray.m_UserTypeMap;
		g_pMain->m_bBotSaveDataArray.m_lock.unlock();
...
void CUser::ReqHandleUserDatabaseRequest(Packet& pkt)
{
...
#if(__VERSION < 2369)
		Packet newpkt(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::KCUPDATE));
	...
	case ProcDbType::AccountInfoShow:
	{
		...
		{
#if(__VERSION < 2369)
			Packet send(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::ACCOUNT_INFO_SAVE));
			send << uint8(1);
			Send(&send);
#endif
		}
	...
	case ProcDbType::ReturnGiveLetterItem:
	{
		...
		g_pMain->m_KnightReturnLetterGiftListArray.m_lock.lock();
		auto m_sKnightReturnLetterGiftListArray = g_pMain->m_KnightReturnLetterGiftListArray.m_UserTypeMap;
		g_pMain->m_KnightReturnLetterGiftListArray.m_lock.unlock();

		foreach(itr, m_sKnightReturnLetterGiftListArray)
		{
	...
	case ProcDbType::CollectionRaceReward:
	{
		...
		_ITEM_TABLE pItemData = g_pMain->GetItemPtr(ItemID);
		if (pItemData.isnull())
			return;
	...
	{
		uint32 itemid, itemprice; std::string name;uint8 sCodeType;
		pkt >> sCodeType >> itemid >> itemprice >> name;
		...
		{
			m_nTLBalance -= itemprice;
			g_pMain->SendHelpDescription(this, string_format("Congratulations, You have gift purchased %s for %d TL.", itemdata.m_sName.c_str(), itemprice).c_str());
		}
		else
			return;
#if(__VERSION < 2369)
		...
		result << uint8(ProcDbType::UpdateKnightCash);
...
void CUser::ReqNtsCommand(Packet& pkt)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (pUser->GetName() == pdata[i].bCharName)
		{
			...
			if (pUser->GetHealth() < (pUser->GetMaxHealth() / 2)) 
				HpChange(pUser->GetMaxHealth());
...
void CUser::ReqAccountLogIn(Packet& pkt)
{
	...
	if (dbresult >= 0 && dbresult <= 2)
	{
		...
		result << uint32(0) << uint8(6);
		Send(&result);
		result.clear();
		result.Initialize(0xC0);
		result << int8(1) << int8(2) << int8(1);
		Send(&result);
		return;
	}
...
void CUser::ReqAllCharInfo(Packet& pkt)
{
	...
	case AlreadyCharName:
	{
		...
		uint8 sResult = g_DBAgent.CharacterSelectLoginChecking(GetAccountName(), sOldCharID);
...
void CUser::ReqCreateNewChar(Packet& pkt)
{
	...
	if (sonuc == 0)
	{
		...
		bool sAutoClan = g_DBAgent.LoadAutoClanInfo(strCharID, newClass, newLevel, newClanID, newFame, newLoyalty, newMontlyLoyalty);
		if ((sAutoClan && newFame == 5) && (newClanID == 1 || newClanID == 15001))
		{
		...
		if (g_pMain->pServerSetting.premiumID && g_pMain->pServerSetting.premiumTime)
			GivePremium((uint8)g_pMain->pServerSetting.premiumID, g_pMain->pServerSetting.premiumTime, true);
		newChar = true;
	}
	Send(&result);
}
