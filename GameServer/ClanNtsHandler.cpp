...
void CUser::ReqClanNts()
{
...

	{
		...
		foreach(itr, m_sKnightsUser)
			if (itr->second)
				m_userlist.push_back(*itr->second);
	}
...
uint8 CGameServerDlg::GetCntNewRace(uint16 newclass, Nation newnation)
{
	...
	if ((classtype == 1 
		...
		|| classtype == 8) 
		&& newnation == Nation::KARUS)
		newrace = KARUS_MIDDLE;
	else if ((classtype == 3
		|| classtype == 9 
		|| classtype == 10)
		&& newnation == Nation::ELMORAD)
		newrace = ELMORAD_WOMAN;
	else if ((classtype == 3
		|| classtype == 9 
		|| classtype == 10) 
		&& newnation == Nation::KARUS)
...
void CUser::ClanNtsSendMsg(cntscode p, std::string strUserID)
{
	...
	if (p == cntscode::Failed) 
		message = "something went wrong.";
	else if (p == cntscode::SuccessReq) 
		message = "The request has been successfully discarded.";
	else if (p == cntscode::AlreadyReq) 
		message = "You have already made a request.";
	else if (p == cntscode::NoClanLeader)
		message = "You are not in a clan or a leader.";
	else if (p == cntscode::OnlineNember)
		message = string_format("Online User %s", strUserID.empty() ? "-" : strUserID.c_str());
	else if (p == cntscode::NoItem) 
		message = "You do not have the required parts on you.";
	else if (p == cntscode::isKing)
		message = string_format("King User %s", strUserID.empty() ? "-" : strUserID.c_str());
	else if (p == cntscode::isClan)
		message = string_format("is in clan User %s", strUserID.empty() ? "-" : strUserID.c_str());
	else if (p == cntscode::Success)
		message = "Success Process";

	if (!title.empty() 
		&& !message.empty()) 
		HSACSX_SendMessageBox(title, message);
}