...
void MagicInstance::Run()
{
	...
	if (pSkillCaster->isPlayer())
	{
		...
		_HSACSX_USER_INFO& p = pUser->pUserMagicUsed.status;
	...
	if (pSkillCaster->isPlayer())
	{
		if ((pSkill.bMoral == MORAL_SELF 
			&& !bIsRunProc)
			|| (pSkill.bMoral == MORAL_FRIEND_WITHME 
			...
			|| pSkill.bMoral == MORAL_AREA_ENEMY)
		{
			...
			if ((!pUser->isGM() 
				...
				&& (!g_pMain->isJuraidMountainActive() 
					|| !pUser->isEventUser()))
				return SendSkillFailed();

			if (!pUser->isGM() 
				&& protoid == DEVA_BIRD_SSID 
				&& pUser->GetZoneID() == ZONE_JURAID_MOUNTAIN
			...
			|| bOpcode == (uint8)MagicOpcode::MAGIC_EFFECTING)
		{
			...
			if (cast != CastCheckType::Add
				&& cast != CastCheckType::Success)
				return SendSkillCasting();

			if (bOpcode == (uint8)MagicOpcode::MAGIC_FLYING)
			{
				bool caught = (pUser->pUserMagicUsed.lastmultiple == 1
					&& isArcher3liOK()) 
					|| (pUser->pUserMagicUsed.lastmultiple == 2 
						&& isArcher5liOK());

				if (pUser->pUserMagicUsed.lastmultipletime > UNIXTIME2 
					&& caught)
					return;
		...
		if (pSkillCaster->isPlayer() && AnimatedSkill())
		{
			...
			if (!returnfail 
				&& bOpcode == (uint8)MagicOpcode::MAGIC_CASTING
				&& pUser->m_sSpeed != 0)
				pUser->pUserMagicUsed.bCastFailed = true;

			bool pos_check = pUser->pUserMagicUsed.castX == pUser->GetSPosX() 
				&& pUser->pUserMagicUsed.castZ == pUser->GetSPosZ();
		...
		if (!isArcher3liOK() 
			...
			&& bOpcode == (uint8)MagicOpcode::MAGIC_FAIL
			&& pSkill.bType[0] == 2
			&& TO_USER(pSkillCaster)->isRogue() 
			&& TO_USER(pSkillCaster)->pUserMagicUsed.archerfailreturn)
		{
	...
	if (pSkillCaster->isPlayer()
		...
		&& TO_USER(pSkillCaster)->isInGenie()
	...
	case MagicOpcode::MAGIC_FLYING:
	{
		...
		if (pSkillCaster && pSkillCaster->isPlayer())
		{
			...
			if (pCaster)
			{
				if (pSkill.sMsp > pCaster->GetMana())
				{
					SendSkillFailed();
					return;
				}
				...
				{
					if (pSkill.sSp > pCaster->GetStamina())
					{
						SendSkillFailed();
						return;
					}
	...
	case MagicOpcode::MAGIC_EFFECTING:
	{
		...
		if (pSkillCaster->isPlayer() && !bIsRecastingSavedMagic)
		{
			...
			if (pMagicCaster->m_bResHpType == USER_SITDOWN 
				&& (pSkill.bMoral == 7 || pSkill.bMoral == 10))
				return SendSkillFailed();
		}
		...
		if (pSkillCaster->isPlayer())
		{
			...
			if (!is_buff)
			{
				if (pSkill.bType[0] != 0)
				{
					...
					{
						pCaster->m_sMagicTypeCooldownListLock.lock();
						auto find = pCaster->m_sMagicTypeCooldownList.find(10);
						if (find != pCaster->m_sMagicTypeCooldownList.end())
							find->second.time = UNIXTIME2;
						else
							pCaster->m_sMagicTypeCooldownList.emplace(std::make_pair(10, _type_cooldown(UNIXTIME2, false)));
						pCaster->m_sMagicTypeCooldownListLock.unlock();
					}
					else
					{
						pCaster->m_sMagicTypeCooldownListLock.lock();
						auto find = pCaster->m_sMagicTypeCooldownList.find(pSkill.bType[0]);
						if (find != pCaster->m_sMagicTypeCooldownList.end())
							find->second.time = UNIXTIME2;
						else
							pCaster->m_sMagicTypeCooldownList.emplace(std::make_pair(pSkill.bType[0], _type_cooldown(UNIXTIME2, false)));
						pCaster->m_sMagicTypeCooldownListLock.unlock();
					}
				}
				...
				{
					pCaster->m_sMagicTypeCooldownListLock.lock();
					auto find = pCaster->m_sMagicTypeCooldownList.find(pSkill.bType[1]);
					if (find != pCaster->m_sMagicTypeCooldownList.end())
						find->second.time = UNIXTIME2;
					else
						pCaster->m_sMagicTypeCooldownList.emplace(std::make_pair(pSkill.bType[1], _type_cooldown(UNIXTIME2, false)));
					pCaster->m_sMagicTypeCooldownListLock.unlock();
				}
				...
				|| is_minor)
			{
				...
				time += UNIXTIME2;
				pCaster->m_sCoolDownListLock.lock();
				auto itr = pCaster->m_sCoolDownList.find(nSkillID);
				if (itr == pCaster->m_sCoolDownList.end())
					pCaster->m_sCoolDownList.emplace(std::make_pair(nSkillID, _cooldown(time, UNIXTIME2)));
				else
				{
					itr->second.recasttime = time;
					itr->second.time = UNIXTIME2;
				}
				pCaster->m_sCoolDownListLock.unlock();
			}
		}
		...
		if (pSkill.bType[0] != 2 && bInstantCast) 
			CMagicProcess::RemoveType4Buff((uint8)BuffType::BUFF_TYPE_INSTANT_MAGIC, pSkillCaster);
	}
	break;
...
CastCheckType MagicInstance::CheckCastTime()
{
	...
	{
		auto itr = pUser->m_sSkillCastList.find(nSkillID);
		if (itr != pUser->m_sSkillCastList.end())
		{
			itr->second.time = UNIXTIME2;
			itr->second.castcheck = false;
			itr->second.flyfail = false;
		}
		else pUser->m_sSkillCastList.insert(std::make_pair(nSkillID, _castlist(UNIXTIME2, false, false)));
	...
	{
		auto itr = pUser->m_sSkillCastList.find(nSkillID);
		if (pUser->m_sSkillCastList.empty() || itr == pUser->m_sSkillCastList.end())
		{
			if (!CheckTbl())
		...
		if (bOp == MagicOpcode::MAGIC_FLYING)
		{
			...
			{ 
				...
				if (pSkill.iNum == 490033) 
					ar = 550;

				if (diff < ar)
				{
					if (pSkill.sReCastTime > 0) 
						SendSkillNoFunction();
			...
			else
			{
				...
				{
					ULONGLONG ar = ULONGLONG(pSkill.bCastTime * katsayi);
					if (pSkill.iNum == 490033) 
						ar = 500;
		...
		else if (bOp == MagicOpcode::MAGIC_EFFECTING)
		{
			...
			pUser->m_sSkillCastList.erase(nSkillID);
			...
			if (pUser->GetFame() == COMMAND_CAPTAIN)
			{
				...
				if (pSkill.bType[0] == 2) 
					ar = 675;
				else if (pSkill.iNum == 490033) 
					ar = 550;
				else if (pSkill.iNum == 106781 
					|| pSkill.iNum == 206781) 
					ar = 600;
			...
			{
				if (diff < (ULONGLONG)pSkill.bCastTime * katsayi)
				{
					if (pSkill.sReCastTime > 0) 
						SendSkillNoFunction();
...
SkillUseResult MagicInstance::UserCanCast()
{
	...
	if (pSkill.iNum >= 300000 && pSkill.iNum <= 400000)
	{
		...
		{
			if (TO_USER(pSkillCaster)->GetZoneID() == ZONE_CHAOS_DUNGEON && nSkillID != 490221 
				|| TO_USER(pSkillCaster)->GetZoneID() == ZONE_CHAOS_DUNGEON && nSkillID != 490332)
				return SkillUseResult::SkillUseFail;
		}
	}
	...
	if (pSkillCaster->isPlayer())
	{
		...
		if (pSkillTarget != nullptr)
		{
			if (pSkillTarget->isNPC())
			{
				if (TO_NPC(pSkillTarget)->m_OrgNation == 3)
					return SkillUseResult::SkillUseFail;
				else
				{
					if (TO_NPC(pSkillTarget)->GetByGroup() == (uint8)Nation::NONE
			...
			if (pSkillCaster->isPlayer())
			{
				if (pSkillCaster->isInTempleQuestEventZone() && (!pSkillCaster->isSameEventRoom(pSkillTarget) && TO_USER(pSkillCaster)->m_sMonsterStoneStatus))
					return SkillUseResult::SkillUseFail;
			}
		}
			...
			&& (pSkill.iUseItem != 0 && !TO_USER(bType == RESURRECTION ? pSkillTarget : pSkillCaster)->CanUseItem(pSkill.iUseItem, (bType == RESURRECTION ? sNeedStone : 1))))
		{
			...
			else if (pSkill.iUseItem == 811071000)
			{
				/*continue*/
			}
			else
		...
		if (pSkill.iNum == 490301
			...
			|| pSkill.iNum == 490307)
		{
			if (pSkillCaster && pSkillCaster->isDead() 
				|| pSkillCaster && pSkillCaster->isBlinking())
				return SkillUseResult::SkillUseFail;
		...
		if (pSkill.iNum == 610090 
			...
			|| pSkill.iNum == 610124)
		{
			if (pSkillCaster && pSkillCaster == pSkillTarget 
				|| pSkillCaster && pSkillCaster->isDead() 
				|| pSkillCaster && pSkillCaster->isBlinking())
				return SkillUseResult::SkillUseFail;

			if (pSkillCaster 
				&& pSkillCaster->GetNation() == (uint8)Nation::KARUS 
				&& pSkillCaster->GetZoneID() != ZONE_KARUS)
				return SkillUseResult::SkillUseFail;
			else if (pSkillCaster 
				&& pSkillCaster->GetNation() == (uint8)Nation::ELMORAD 
				&& pSkillCaster->GetZoneID() != ZONE_ELMORAD)
				return SkillUseResult::SkillUseFail;
		...
		if (pSkill.bType[0] == 6)
		{
			...
			{
				if (!pSkillCaster->isTransformationMonsterInZone())
					return SkillUseResult::SkillUseFail;
			}
	...
	if (TO_USER(pSkillCaster)->GetClass() == 212 
		|| TO_USER(pSkillCaster)->GetClass() == 211 
		|| TO_USER(pSkillCaster)->GetClass() == 204
		|| TO_USER(pSkillCaster)->GetClass() == 104 
		|| TO_USER(pSkillCaster)->GetClass() == 111 
		|| TO_USER(pSkillCaster)->GetClass() == 112)
	{
		if (nSkillID == 111703 
			|| nSkillID == 211703
			|| nSkillID == 112703
		...
		{
			if (pSkillTarget != nullptr 
				&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)40)
		...
		|| TO_USER(pSkillCaster)->GetClass() == 112)
	{
		...
		if (pSkillTarget != nullptr 
			&& pSkill.bType[0] == 1 
			&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)19)
			return SkillUseResult::SkillUseFail;
	}

	if (TO_USER(pSkillCaster)->GetClass() == 201 
		|| TO_USER(pSkillCaster)->GetClass() == 101
		|| TO_USER(pSkillCaster)->GetClass() == 205
		|| TO_USER(pSkillCaster)->GetClass() == 105 
		|| TO_USER(pSkillCaster)->GetClass() == 206 
		|| TO_USER(pSkillCaster)->GetClass() == 106)
	{
		...
		{
			if (pSkill.bType[0] == 1
				&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)50)
			  return SkillUseResult::SkillUseFail;
		}
		else
		{
	...
	if (TO_USER(pSkillCaster)->GetClass() == 203 
		|| TO_USER(pSkillCaster)->GetClass() == 103
		|| TO_USER(pSkillCaster)->GetClass() == 209
		|| TO_USER(pSkillCaster)->GetClass() == 109 
		|| TO_USER(pSkillCaster)->GetClass() == 210 
		|| TO_USER(pSkillCaster)->GetClass() == 110)
	{
		if (nSkillID == 109542 
			|| nSkillID == 209542
			|| nSkillID == 110542 
	...
	if (TO_USER(pSkillCaster)->GetClass() == 102 
		|| TO_USER(pSkillCaster)->GetClass() == 202
		|| TO_USER(pSkillCaster)->GetClass() == 107
		|| TO_USER(pSkillCaster)->GetClass() == 108 
		|| TO_USER(pSkillCaster)->GetClass() == 207 
		|| TO_USER(pSkillCaster)->GetClass() == 208)
	{
		if (pSkillTarget != nullptr
			&& nSkillID != 208575 
			&& nSkillID != 108575 
			&& pSkill.bType[0] == 1 
			&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)23)
			return SkillUseResult::SkillUseFail;
		...
		{
			if (pSkillTarget != nullptr 
				&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)17)
				return SkillUseResult::SkillUseFail;
		}
		...
		{
			if (pSkillTarget != nullptr 
				&& pSkillCaster->GetDistanceSqrt(pSkillTarget) > (float)34)
				return SkillUseResult::SkillUseFail;
		}
	}
	...
	if (pSkillCaster->isNPC())
	{
		CNpc* pNpc = TO_NPC(pSkillCaster);
		if (pNpc)
		{
			if (pNpc->GetMana() - pSkill.sMsp < 0)
				return SkillUseResult::SkillUseFail;

			pNpc->MSpChange(-(pSkill.sMsp));
		}
	}
...
SkillUseResult MagicInstance::CheckSkillPrerequisites()
{
		...
		if (pCaster)
		{
				...
				&& bOpcode != (uint8)MagicOpcode::MAGIC_FAIL)
			{
				...
				if (itr != pCaster->m_sCoolDownList.end())
				{
					...
					//pCaster->m_sCoolDownList.erase(nSkillID);
				}
				pCaster->m_sCoolDownListLock.unlock();
			}
			...
			if (pCaster->isRogue() && pSkill.iUseItem != 370007000 && pCaster->GetZoneID() != ZONE_CHAOS_DUNGEON)
			{
				if (pSkill.bType[0] == 2 || (pSkill.bType[0] == 1 && !isEskrimaSkills()))
				{
					...
					if (!pLeftHand.isnull())
					{
						...
						if (!isBlowArrowSkills() 
							&& pLeftHand.isBow() 
							&& (pSkill.bType[0] == 1 || pSkill.bType[1] == 1) 
							&& pSkill.bCastTime == 0)
							return SkillUseResult::SkillUseFail;
					}
				...
				&& (MagicOpcode)bOpcode != MagicOpcode::MAGIC_FAIL)
			{
				...
				pCaster->m_sMagicTypeCooldownListLock.lock();
				auto itr = pCaster->m_sMagicTypeCooldownList.find(pSkill.bType[0]);
				if (itr != pCaster->m_sMagicTypeCooldownList.end())
				{
					...
					//pCaster->m_sMagicTypeCooldownList.erase(pSkill.bType[0]);
				}
				pCaster->m_sMagicTypeCooldownListLock.unlock();
			}
			...
			{
				pCaster->m_sMagicTypeCooldownListLock.lock();
				auto itr = pCaster->m_sMagicTypeCooldownList.find(pSkill.bType[1]);
				if (itr != pCaster->m_sMagicTypeCooldownList.end())
				{
	...
	if (pSkillTarget != nullptr)
	{
		...
		if (pSkillTarget->isNPC() 
			&& pSkillCaster 
			&& !pSkillCaster->isAttackable(pSkillTarget))
			return SkillUseResult::SkillUseFail;
...
void MagicInstance::BuildSkillPacket(Packet& result, int16 sSkillCaster, int16 sSkillTarget, int8 opcode, uint32 nSkillID, int16 sData[7])
{
	...
	result << opcode << nSkillID << uint32(sSkillCaster) << uint32(sSkillTarget) << uint32(sData[0]) << uint32(sData[1]) << uint32(sData[2]) << uint32(sData[3]) << uint32(sData[4]) << uint32(sData[5]) << uint32(sData[6]);
}
		...
		if (pSkillCaster)
		{
			...
			if (pSkillTarget)
			{
				...
				if (pSkillCaster->isPlayer())
				{
					...
					{
						CUser* pUser = TO_USER(pSkillCaster);
						CBot* pTUser = TO_BOT(pSkillTarget);
						...
						{
							// If the caster's not in a party, make sure the target's not someone other than themselves.
							if ((!pUser->isInParty() && pUser->GetName() != pTUser->GetName())
								// Also ensure that if there is a target, they're in the same party.
								|| (pTUser->isInParty() && pTUser->GetPartyID() != pUser->GetPartyID()))
								goto fail_return;
						}
				...
				else if (pSkillCaster->isBot())
				{
					if (pSkillTarget->isPlayer())
					{
						CBot* pUser = TO_BOT(pSkillCaster);
						CUser* pTUser = TO_USER(pSkillTarget);
	...
	case MORAL_CLAN:
	{
		...
		if (pSkillCaster)
		{
			...
			if (pSkillTarget)
			{
				...
				if ((!pUser->isInClan() && pUser != pTUser)
					// If we're targeting someone, that target must be in our clan.
					|| (pTUser != nullptr
						&& pTUser->GetClanID() != pUser->GetClanID()))
					goto fail_return;
			}
		}
	} break;
	...
	if (pSkillCaster)
	{
		if (pSkillCaster->isPlayer())
		{
			...
			if (pUser)
			{
				...
				//printf("Yüzdeli İşlem %u\n", modulator);
				if (modulator != 0) {
					Class = pSkill.sSkill / 10;
					if (Class != pUser->GetClass()) goto fail_return;
					if ((pSkill.sSkillLevel > pUser->m_bstrSkill[modulator]) && pUser->GetFame() != COMMAND_CAPTAIN) goto fail_return;
				}

				else if (pSkill.sSkillLevel > pUser->GetLevel()
				...
				if (pSkill.bType[0] == 3)
				{
					...
					if (!pUser->canUsePotions()
						...
						&& (pTable = g_pMain->GetItemPtr(pSkill.iUseItem)).isnull()
				...
				if (pSkill.bType[0] == 1)
				{
					...
					else if (pSkill.sSkill == 1056 || pSkill.sSkill == 2056)
					{
						...
						if (pUser->GetItem(LEFTHAND)->nNum != 0)
							return false;
				...
				{
					int total_hit = pUser->m_sTotalHit;

					if (pSkill.bType[0] == 2 && pSkill.bFlyingEffect != 0) // Type 2 related...
						return true;		// Do not reduce MP/SP when flying effect is not 0.

					if (pSkill.sMsp > pUser->GetMana())
						goto fail_return;

					if (pUser->isPortuKurian())
					{
						if (pSkill.sSp > pUser->GetStamina())
							goto fail_return;
					}

					// If the PLAYER uses an item to cast a spell.
					if (pSkill.bType[0] == 3 || pSkill.bType[0] == 4)
					{
						if (pSkill.iUseItem != 0) {
							_ITEM_TABLE pItem;				// This checks if such an item exists.
							pItem = g_pMain->GetItemPtr(pSkill.iUseItem);
							if (pItem.isnull()) return false;

							if ((pItem.m_bClass != 0 && !pUser->JobGroupCheck(pItem.m_bClass))
								|| (pItem.m_bReqLevel != 0 && pUser->GetLevel() < pItem.m_bReqLevel))
								return false;
						}
					}

					if ((pSkill.bType[0] != 4 || (pSkill.bType[0] == 4 && sTargetID == -1)) && !pUser->isBlinking())
					{
						if (pUser->isDevil()
							&& pUser->CheckSkillPoint(PRO_SKILL3, 45, 83))
						{
							int16 nDivide = pSkill.sMsp / 2;
							pUser->MSpChange(-(nDivide));
						}
						else
							pUser->MSpChange(-(pSkill.sMsp));

						if (pUser->isPortuKurian())
							pUser->SpChange(-(pSkill.sSp));
					}
					...
					{
						if (pSkill.sHP > pUser->GetHealth())
							goto fail_return;

						pUser->HpChange(-pSkill.sHP);
					}
...
bool MagicInstance::ExecuteType1()
{
	...
	if (pSkillTarget != nullptr)
	{
		...
		if (pSkillTarget->isPlayer())
			TO_USER(pSkillTarget)->ItemWoreOut(DEFENCE, damage);
	...
	else if (sTargetID == -1)
	{
		...
		// nxwile yere vurma düzenlemesi 28.06.2024
		...
		// nxwile yere vurma düzenlemesi 28.06.2024 end
		...
		foreach(itr, casted_member)
		{
			...
			if (pSkill.sRange > 0 && pSkillCaster->isPlayer()) {
				...
				// // nxwile warrior skill vurma fix yere vurma düzenlemesi 28.06.2024
				if (isStompSkills())
				{
	...
	*/
	...
	if (pSkillCaster->isPlayer())
	{
		...
		if (pSkillCaster->isPlayer())
		{
			if (pSkillTarget 
				&& pSkillTarget->GetX() > 0 
				&& !pSkillCaster->isInRange(pSkillTarget->GetX(), pSkillTarget->GetZ(), 2400.0F))
				goto packet_send;
		}
...
bool MagicInstance::ExecuteType3()
{
	...
	if (sTargetID == -1)
	{
		...
		foreach(itr, unitList)
		{
			...
			if (pTarget->isPlayer() 
				&& (TO_USER(pTarget)->isGM() || TO_USER(pTarget)->isBlinking()))
				continue;
	...
	if (pType && pType->bDirectType == 18)
	{
		...
		if (!pSkillCaster->isPlayer() 
			|| pSkillCaster && !TO_USER(pSkillCaster)->hasFullAngerGauge())
			return false;
	...
	foreach(itr, casted_member)
	{
		...
		if (pSkillCaster->isPlayer())
		{
				...
				|| nSkillID == 210743)
			{
				if (pSkillTarget 
					&& pSkillTarget->GetX() > 0 
					&& !pSkillCaster->isInRange(pSkillTarget->GetX(), pSkillTarget->GetZ(), 135.0f))
					continue;
			}
				...
				|| nSkillID == 210757)
			{
				if (pSkillTarget 
					&& pSkillTarget->GetX() > 0 
					&& !pSkillCaster->isInRange(pSkillTarget->GetX(), pSkillTarget->GetZ(), 5250.0F))
					continue;
			}
				...
				|| nSkillID == 210718)
			{
				if (pSkillTarget 
					&& pSkillTarget->GetX() > 0 
					&& !pSkillCaster->isInRange(pSkillTarget->GetX(), pSkillTarget->GetZ(), 4150.0f))
					continue;
			}
			...
			{
				if (pTarget->GetX() > 0 
					&& !pSkillCaster->isInRange(pTarget->GetX(), pTarget->GetZ(), 2250.0F))
					continue;
			}
		}
		else if (pSkillCaster->isBot())
		...
		if (pType 
			...
			&& (pType->bDirectType == 1 || pType->bDirectType == 8) && (nSkillID < 400000) 
		...
		if (pType 
			&& pType->bDirectType < 2 && nSkillID < 400000 
			&& pTarget 
			&& pTarget->isPlayer()
			&& (pType->bAttribute == (uint8)AttributeType::AttributeLightning || pType->bAttribute == (uint8)AttributeType::AttributeIce))
		...
		if (pType->bDuration == 0)
		{
			...
			case 1:
			{
					...
					&& damage > 0)
				{
					...
					if (hpfark > 0)
					{
						...
						{
							int percent = int(asec * 100); uint8 assistype = 0;
			...
			case 21:
					...
					&& pTarget->isPlayer())
				{
					...
					uint32 Noah = pItem.m_iBuyPrice;
					if (Noah > 0 
						&& !TO_USER(pTarget)->hasCoins(Noah))
						return false;

					if (Noah > 0 
						&& !TO_USER(pSkillCaster)->GoldLose(Noah))
						return false;
...
bool MagicInstance::ExecuteType4()
{
	...
	if (sTargetID == -1)
	{
		...
		foreach(itr, unitList)
		{
			...
			if (pTarget->isPlayer() 
				&& (TO_USER(pTarget)->isGM() || !TO_USER(pTarget)->isInGame() || TO_USER(pTarget)->isBlinking()) 
				|| pTarget->isDead()) 
				continue;
			...
			if (pTarget->isNPC() 
				&& (pSkill.bMoral == 7 || pSkill.bMoral == 10)
				&& (pType->bBuffType == 40 || pType->bBuffType == 6 || pType->bBuffType == 47)) 
				added = 3;
	...
	{
		if (pSkillTarget == nullptr 
			|| (pSkillTarget && pSkillTarget->isPlayer() && pSkillTarget->isDead()) 
			|| (pSkillTarget && pSkillTarget->isPlayer() && TO_USER(pSkillTarget)->isBlinking() && !bIsRecastingSavedMagic))
			return false;
	...
	foreach(itr, casted_member)
	{
		...
		else if (pTarget->isBot()) // Update Bot stats.
		{
			TO_BOT(pTarget)->SetBotAbility();

			if (pType->isBuff() && pType->bBuffType == BUFF_TYPE_HP_MP && bIsRecastingSavedMagic)
				TO_BOT(pTarget)->HpChange(pTarget->m_sMaxHPAmount);
		...
		if (pType && pType->bTargetChange == 1)
		{
			if (pSkillCaster->isNPC())
			{
				if (TO_NPC(pSkillCaster)->isPet())
				{
					...
					{
						if (pUser->m_PettingOn->isFamilyAttack())
							return false;
...
bool MagicInstance::ExecuteType8()
{
	...
	foreach(itr, casted_member)
	{
		...
		case 29:
		{
			...
			_SOCCER_STATUS_INFO* pRoomInfo = g_pMain->m_TempleSoccerEventRoomList.GetData(TO_USER(pSkillCaster)->GetZoneID());
			if (pRoomInfo != nullptr)
			{
				if (pRoomInfo->isSoccerAktive())
					pRoomInfo->m_SoccerSocketID = TO_USER(pSkillCaster)->GetSocketID();
				else
				{
					sData[1] = 0;
					BuildAndSendSkillPacket(pSkillCaster, true, sCasterID, (*itr)->GetID(), bOpcode, nSkillID, sData);
			...
			if (pSkillCaster->isPlayer())
			{
				...
				if (pTUser->isNPC())
				{
					CNpc* pNpc = TO_NPC(pTUser);
					if (pNpc == nullptr)
						return false;

					warp_x /= distance; warp_z /= distance;
					warp_x *= pType->sRadius; warp_z *= pType->sRadius;
					warp_x += pUser->m_oldx; warp_z += pUser->m_oldz;

					sData[1] = 1;
					BuildAndSendSkillPacket(*itr, true, sCasterID, (*itr)->GetID(), bOpcode, nSkillID, sData);
					pNpc->SendMoveResult(warp_x, 0, warp_z, distance);
				}
			}
		}
		break;
...
short MagicInstance::GetMagicDamage(Unit* pTarget, int total_hit, int attribute)
{
		...
		if (pSkillCaster->isPlayer())
		{
			...
			if (pUser->isMage())
			{
				if (pTarget->isPlayer())
					...
					bStat = pTarget->isBot() ? pUser->GetStat(StatType::STAT_CHA) : pUser->GetStatWithItemBonus(StatType::STAT_CHA);
				else
	...
	if (result != FAIL)
	{
		...
		if (pSkillCaster->isPlayer())
		{
			...
			if (!pUser->isWeaponsDisabled() && !pRightHand.isnull() && pRightHand.isStaff() && pUser->GetItemPrototype(LEFTHAND).isnull())
			{
				...
				auto itr = pUser->m_sEquippedItemBonuses.find(RIGHTHAND);
				if (itr != pUser->m_sEquippedItemBonuses.end())
				{
			...
			int16 sAmount = 0;
			Guard lock(pUser->m_sEquippedItemBonusLock);
		...
		if (pTarget->isPlayer())
		{
			...
			if (!isChaSkill)
			{
				if (pSkillCaster->isBot())
				{
					// Now handle class-specific AC/AP bonuses.
					temp_ac = temp_ac * (100 + TO_USER(pTarget)->m_byAcClassBonusAmount[TO_BOT(pSkillCaster)->GetBaseClass() - 1]) / 100;
					temp_ap = temp_ap * (100 + TO_BOT(pSkillCaster)->m_byAPClassBonusAmount[TO_USER(pTarget)->GetBaseClass() - 1]) / 100;
				}
				else if (pSkillCaster->isPlayer())
				{
					// Now handle class-specific AC/AP bonuses.
					temp_ac = temp_ac * (100 + TO_USER(pTarget)->m_byAcClassBonusAmount[TO_USER(pSkillCaster)->GetBaseClass() - 1]) / 100;
				...
				temp_hit = (int32)(temp_hit_B * (damage / 100.0f));
				damage = temp_hit;
			}
		}
		else if (pTarget->isBot())
		{
			...
			if (!isChaSkill)
			{
				...
				{
					// Now handle class-specific AC/AP bonuses.
					temp_ac = temp_ac * (100 + TO_BOT(pTarget)->m_byAcClassBonusAmount[TO_BOT(pSkillCaster)->GetBaseClass() - 1]) / 100;
					temp_ap = temp_ap * (100 + TO_BOT(pSkillCaster)->m_byAPClassBonusAmount[TO_BOT(pTarget)->GetBaseClass() - 1]) / 100;
				}
				else if (pSkillCaster->isPlayer())
				{
					// Now handle class-specific AC/AP bonuses.
					temp_ac = temp_ac * (100 + TO_BOT(pTarget)->m_byAcClassBonusAmount[TO_USER(pSkillCaster)->GetBaseClass() - 1]) / 100;
					temp_ap = temp_ap * (100 + TO_USER(pSkillCaster)->m_byAPClassBonusAmount[TO_BOT(pTarget)->GetBaseClass() - 1]) / 100;
				}
		...
		else
		{
			...
			double dm = (double)damage;
			dm *= g_pMain->pDamageSetting.montakedamage;
			damage = (int32)dm;
		...
		else if (pSkillCaster->isBot())
		{
			if (pTarget->isNPC())
			{
				if (TO_BOT(pSkillCaster)->isWarrior())
					damage *= int32(0.50f);
			}
		}
	...
	if (pSkillCaster->isPlayer())
	{
		...
		if (damage)
		{
			if (TO_USER(pSkillCaster)->isMage())
			{
	...
	else if (pSkillCaster->isBot())
	{
		...
		if (damage)
		{
			if (TO_BOT(pSkillCaster)->isMage())
			{
				baa *= g_pMain->pDamageSetting.magemagicdamage;
				baa *= TO_BOT(pSkillCaster)->getplusdamage();
			}
		}
	}
...
float CUser::getplusdamage()
{
	...
	if (!pLeftHand.isnull())
	{
		if (pLeftHand.m_ItemType == 4 
			...
			&& (pLeftHand.ItemClass == 0 || pLeftHand.ItemClass == 1))
			return g_pMain->pDamageSetting.lowclassitem;
		else if (pLeftHand.m_ItemType == 5 
			&& (pLeftHand.ItemClass == 2 || pLeftHand.ItemClass == 7 || pLeftHand.ItemClass == 33)) 
			return g_pMain->pDamageSetting.middleclassitem;
		else if (pLeftHand.m_ItemType == 5
			&& (pLeftHand.ItemClass == 3 || pLeftHand.ItemClass == 4 || pLeftHand.ItemClass == 8 || pLeftHand.ItemClass == 34)) 
	...
	else
	{
		...
		if (!pRightHand.isnull())
		{
			if (pRightHand.m_ItemType == 4 
				...
				&& (pRightHand.ItemClass == 0 || pRightHand.ItemClass == 1)) 
				return g_pMain->pDamageSetting.lowclassitem;
			else if (pRightHand.m_ItemType == 5 
				&& (pRightHand.ItemClass == 2 || pRightHand.ItemClass == 7 || pRightHand.ItemClass == 33)) 
				return g_pMain->pDamageSetting.middleclassitem;
			else if (pRightHand.m_ItemType == 5 
				&& (pRightHand.ItemClass == 3 || pRightHand.ItemClass == 4 || pRightHand.ItemClass == 8 || pRightHand.ItemClass == 34))
				return g_pMain->pDamageSetting.highclassitem;
...
void MagicInstance::Type9Cancel(bool bRemoveFromMap)
{
	...
	{
		CBot* pCaster = TO_BOT(pSkillCaster);
