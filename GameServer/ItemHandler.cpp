...
bool CUser::CheckExistItem(int itemid, short count /*= 1*/)
{
	if (itemid == ITEM_HUNT)
		...
		return true;
	else if (itemid == ITEM_CHAT)
		return true;
	else if (itemid == ITEM_COUNT)
		return true;
	else
	{
...
#if(GAME_SOURCE_VERSION == 2369)
	ClearUser->HandleHShieldSoftwareNewItemMoveLoadderHandler();
#endif
	result.clear();
...
void CUser::InventorySystemReflesh(Packet & pkt)
{
...
#if(__VERSION < 2369)
#if(GAME_SOURCE_VERSION == 2369)
		sSlot[k - SLOT_MAX] = HandleHShieldRightClickExchangeHandler(m_sItemArray[k].nNum);
	...
	for (int k = 0; k < HAVE_MAX; k++)
		result << sSlot[k] << sSubCode[k] << sItemID[k];
...
void CUser::InventorySystem(Packet & pkt)
{
	...
	//printf("ItemMove %s %d %d %d %d %d\n",GetName().c_str(), dir, nItemID, m_sItemArray[SLOT_MAX + bSrcPos].nNum, bSrcPos, bDstPos);
	bool m_byInventorySlot = false;
	...
	case ITEM_MBAG_TO_MBAG:
		if (bDstPos >= MBAG_TOTAL || bSrcPos >= MBAG_TOTAL
			...
			|| (INVENTORY_MBAG + bDstPos < INVENTORY_MBAG2 && m_sItemArray[CBAG1].nNum == 0)
			|| (INVENTORY_MBAG + bDstPos >= INVENTORY_MBAG2 && m_sItemArray[CBAG2].nNum == 0)
	...
	case ITEM_COSP_TO_INVEN:
		if (bDstPos >= HAVE_MAX || bSrcPos >= COSP_MAX
			...
			|| (bSrcPos == COSP_EMBLAM && nItemID != m_sItemArray[CEMBLEM].nNum)
			|| (bSrcPos == COSP_TALISMAN && nItemID != m_sItemArray[CTALISMAN].nNum)
			|| bSrcPos == COSP_BAG1
			|| bSrcPos == COSP_BAG2)
			goto fail_return;
		...
		else if (bSrcPos == COSP_EMBLAM)
			pSrcItem = &m_sItemArray[CEMBLEM];
		else if (bSrcPos == COSP_TALISMAN)
			pSrcItem = &m_sItemArray[CTALISMAN];
		else
		...
		if (bSrcPos == COSP_FAIRY && pSrcItem->nNum == ITEM_OREADS) m_bFairyCheck = false;
	...
	case ITEM_INVEN_TO_COSP:
		...
		else if(bDstPos == COSP_EMBLAM)
			pDstItem = &m_sItemArray[CEMBLEM];
		else if (bDstPos == COSP_TALISMAN)
			pDstItem = &m_sItemArray[CTALISMAN];
		else
			pDstItem = &m_sItemArray[INVENTORY_COSP + bDstPos];

		if (bDstPos == COSP_FAIRY && pSrcItem->nNum == ITEM_OREADS) m_bFairyCheck = true;
		break;
	...
	case ITEM_INVEN_SLOT:
		...
		if (pTable.m_bSlot == 0x01 || (pTable.m_bSlot == 0x00 && bDstPos == RIGHTHAND))
		{
			if (pLeftHand->nNum != 0)
			{
				_ITEM_TABLE pTable2 = g_pMain->GetItemPtr(pLeftHand->GetNum());
				if (pTable2.isnull())
					goto fail_return;
		...
		else if (pTable.m_bSlot == 0x02 || (pTable.m_bSlot == 0x00 && bDstPos == LEFTHAND))
		{
			...
			{
				_ITEM_TABLE pTable2 = g_pMain->GetItemPtr(pRightHand->GetNum());
				if (pTable2.isnull())
					goto fail_return;
		...
		{
			if (pLeftHand->nNum != 0 && pRightHand->nNum != 0) 
				goto fail_return;
			else if (pLeftHand->nNum != 0)
			{
				memcpy(pRightHand, pSrcItem, sizeof(_ITEM_DATA));
				memcpy(pSrcItem, pLeftHand, sizeof(_ITEM_DATA));
				memset(pLeftHand, 0, sizeof(_ITEM_DATA));
	...
	case ITEM_PET_TO_INVEN:
		if (m_PettingOn == nullptr 
			|| bSrcPos >= PET_INVENTORY_TOTAL 
			|| bDstPos >= HAVE_MAX 
			|| nItemID != m_PettingOn->sItem[bSrcPos].nNum)
			goto fail_return;
	...
	case ITEM_SLOT_INVEN_TO_MBAG:
		if (bDstPos >= COSP_MAX + MBAG_COUNT || bSrcPos >= HAVE_MAX
			// Make sure that the item actually exists there.
			|| nItemID != m_sItemArray[SLOT_MAX + bSrcPos].nNum
			|| !IsValidSlotPos(pTable, bDstPos + 8))
			goto fail_return;

		pSrcItem = &m_sItemArray[SLOT_MAX + bSrcPos];
		pDstItem = &m_sItemArray[INVENTORY_COSP + bDstPos + 8];
	...
	if (dir == ITEM_INVEN_SLOT 
		...
		|| dir == ITEM_INVEN_TO_COSP
		|| dir == ITEM_COSP_TO_INVEN
		|| dir == ITEM_SLOT_SLOT)
	{
...
#define MAX_DISPLAY 8
		...
		foreach(HSACSXSoftdom, m_exchangelist) {

			uint32 nItemID = HSACSXSoftdom->ItemID;
			uint32 nCount = HSACSXSoftdom->ItemCount;
...
{
	if (isQuest)
		return RunQuestExchange(nExchangeID, SelectedAward);
...
bool CUser::IsValidSlotPos(_ITEM_TABLE pTable, int destpos)
{
	...
	case ItemSlotBag:
		if (destpos != COSP_BAG1 + 3 && destpos != COSP_BAG2)
			return false;
		break;
	...
	case ItemSlotCospreTalisman:
		if (destpos != COSP_TALISMAN)
			return false;
		break;

	case ItemSlotCospre0Emblem:
	case ItemSlotCospre1Emblem:
		if (destpos != COSP_EMBLAM)
			return false;
		break;
