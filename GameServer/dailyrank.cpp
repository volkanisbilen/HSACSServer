...
void CUser::HandleDailyRankShow(Packet& pkt)
{
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::GRAND_MERCHANT]) 
			return;

		g_pMain->m_DailyRank.m_lock.lock();
		auto m_DailyRank = g_pMain->m_DailyRank.m_UserTypeMap;
		g_pMain->m_DailyRank.m_lock.unlock();

		foreach(ita, m_DailyRank)
			if (ita->second)
				pDailyList.emplace_back(_DAILY(ita->second->strUserID, ita->second->GmRank[0]));

		if (pDailyList.empty())
		{ 
			newpkt << uint32(0) << uint32(0) << uint16(0); 
			Send(&newpkt); 
			return; 
		}
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::MONSTER_HUNDER]) 
			return;
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::SHOZIN]) 
			return;
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::KNIGHT_ADONIS]) 
			return;
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::HERO_OF_CHAOS]) 
			return;
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::DISCIPLE_KERON]) 
			return;
	...
	{
		if (m_sDailyRankReflesh[(int)DailyRankType::KING_OF_FELANKOR]) return;
		struct _clanlist { std::string clanname = ""; uint32 points = 0; uint16 clanid; _clanlist(std::string a, uint32 b, uint16 c) { clanname = a; points = b; clanid = c; } };
		...
		for (int nation = KARUS_ARRAY; nation <= ELMORAD_ARRAY; nation++) 
		{
			...
			auto m_KnightsRatingArray = g_pMain->m_KnightsRatingArray[nation].m_UserTypeMap;
	...
	case DailyRankType::DRAKI_RANK:
		if (m_sDailyRankReflesh[(int)DailyRankType::DRAKI_RANK]) return;
