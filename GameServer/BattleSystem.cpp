...
void CGameServerDlg::CheckNationMonumentRewards()
{
	...
	auto m_sNationMonumentWinnerNationArray = m_NationMonumentWinnerNationArray.m_UserTypeMap;
	m_NationMonumentWinnerNationArray.m_lock.unlock();

	foreach(itr, m_sNationMonumentWinnerNationArray) 
	{
		_MONUMENT_INFORMATION* pWinnerMonu = itr->second;
		if (pWinnerMonu == nullptr 
			|| pWinnerMonu->RepawnedTime - int32(UNIXTIME) > 0)
			continue;
	...
	auto m_sNationMonumentDefeatedNationArray = m_NationMonumentDefeatedNationArray.m_UserTypeMap;
	m_NationMonumentDefeatedNationArray.m_lock.unlock();

	foreach(itr, m_sNationMonumentDefeatedNationArray) 
	{
		_MONUMENT_INFORMATION* pDefeatedMonu = itr->second;
		if (pDefeatedMonu == nullptr 
			|| pDefeatedMonu->RepawnedTime - int32(UNIXTIME) > 0)
			continue;
	...
	foreach(itr, m_defeatlist)
	{
		...
		foreach(itrx, unitList) 
		{
			...
			if (itr->sSid == LUFERSON_MONUMENT_SID 
				|| itr->sSid == ELMORAD_MONUMENT_SID)
				TO_USER(pTarget)->SendLoyaltyChange("Nation Monument Reward", 200);
			else
				TO_USER(pTarget)->SendLoyaltyChange("Nation Monument Reward", 50);
		}
...
{
	m_WarBanishOfWinnerArray.m_lock.lock();
	auto m_sWarBanishOfWinnerArray = m_WarBanishOfWinnerArray.m_UserTypeMap;
	m_WarBanishOfWinnerArray.m_lock.unlock();

	foreach(itr, m_sWarBanishOfWinnerArray)
	{
	...
	for (uint16 i = 0; i < MAX_USER; i++)
	{
		...
		if (m_byBattleZoneType == BATTLEZONE_OPEN)
		{
			if (nType == BATTLEZONE_OPEN)
				result << uint8(GOLDSHELL) << uint8(1) << uint32(pUser->GetSocketID());
			else
				result << uint8(GOLDSHELL) << uint8(0) << uint32(pUser->GetSocketID());
		}
		else
			result << uint8(GOLDSHELL) << uint8(0) << uint32(pUser->GetSocketID());
