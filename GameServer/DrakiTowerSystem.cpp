...
uint8 CUser::SelectDrakiRoom()
{
	...
	g_pMain->m_DrakiRoomListArray.m_lock.lock();
	auto m_sDrakiRoomListArray = g_pMain->m_DrakiRoomListArray.m_UserTypeMap;
	g_pMain->m_DrakiRoomListArray.m_lock.unlock();

	foreach(itr, m_sDrakiRoomListArray)
	{
...
void CUser::SummonDrakiMonsters(uint8 RoomIndex)
{
	...
	g_pMain->m_DrakiMonsterListArray.m_lock.lock();
	auto m_sDrakiMonsterListArray = g_pMain->m_DrakiMonsterListArray.m_UserTypeMap;
	g_pMain->m_DrakiMonsterListArray.m_lock.unlock();

	foreach(itr, m_sDrakiMonsterListArray)
	{
		DRAKI_MONSTER_LIST* pDrakiMonster = itr->second;

		if (pDrakiMonster == nullptr 
			|| pDrakiMonster->bDrakiStage != RoomIndex)
			continue;
