...
{
	uint32 sNpcID; uint32 nShadowPiece; uint8 nShadowPieceSlot, nMaterialCount, nDownFlag;
	...
	if (nShadowPiece == ITEM_SHADOW_PIECE) 
	{
		...
		if (pShadowItem == nullptr 
			|| pShadowItem->nNum != ITEM_SHADOW_PIECE 
			|| pShadowItem->sCount < 1)
			return ShozinSendFail();

		if (pShadowItem->isBound() 
			|| pShadowItem->isDuplicate() 
			|| pShadowItem->isRented() 
			|| pShadowItem->isSealed())
			return ShozinSendFail();
	...
	auto m_sItemSpecialExchangeArray = g_pMain->m_ItemSpecialExchangeArray.m_UserTypeMap;
	g_pMain->m_ItemSpecialExchangeArray.m_lock.unlock();

	foreach(itr, m_sItemSpecialExchangeArray)
	{
	...
	TRACE("Trina %u %u\n", isShadowPiece, pExchange.isShadowSucces);
