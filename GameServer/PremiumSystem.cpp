...
uint16 CUser::GetPremiumProperty(PremiumPropertyOpCodes type)
{
	...
	case PremiumPropertyOpCodes::PremiumExpPercent:
		{
			g_pMain->m_PremiumItemExpArray.m_lock.lock();
			auto m_sPremiumItemExpArray = g_pMain->m_PremiumItemExpArray.m_UserTypeMap;
			g_pMain->m_PremiumItemExpArray.m_lock.unlock();

			foreach(itr, m_sPremiumItemExpArray)
			{
...
void CUser::GiveClanPremium(uint8 bPremiumType, uint32 sPremiumDay)
{
	...
	auto m_arKnightsUser = pKnights->m_arKnightsUser.m_UserTypeMap;
	pKnights->m_arKnightsUser.m_lock.unlock();

	foreach(i, m_arKnightsUser)
	{
