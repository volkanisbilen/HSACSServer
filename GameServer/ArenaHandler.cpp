...
void CUser::HandleChallengeAcceptCVC(Packet & pkt)
{
	...
	auto m_sKnightsUser = pClan1->m_arKnightsUser.m_UserTypeMap;
	...
	m_sKnightsUser = pClan2->m_arKnightsUser.m_UserTypeMap;
	...
	foreach(itr, pKnights1) 
	{
		...
		if (pTUser->GetZoneID() == GetZoneID()
			&& !pTUser->isMerchanting() 
			&& !pTUser->isTrading()
			&& !pTUser->isMining() 
			&& !pTUser->isFishing() 
			&& !pTUser->isInParty()
			&& !pTUser->m_bRequestingChallenge 
			&& pTUser->m_sChallengeUser < 0)
