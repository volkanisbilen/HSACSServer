...
void CMagicProcess::MagicPacket(Packet& pkt, Unit* pCaster /*= nullptr*/)
{
	...
	if (pCaster->isPlayer() 
		&& !TO_USER(pCaster)->isInGame())
		return;

	MagicInstance instance;
	pkt >> instance.bOpcode >> instance.nSkillID;
	if (instance.nSkillID == 0 
		|| (pCaster->isPlayer()
			&& instance.bOpcode < 5
			&& TO_USER(pCaster)->m_bZoneChangeFlag))
		return;
	...
	int32	sCasterID, sTargetID, sData[7];
	pkt >> sCasterID >> sTargetID;
	for (int i = 0; i < 7; i++)
		pkt >> sData[i];

	if (sTargetID != -1 
		&& (instance.nSkillID == 109035 
			|| instance.nSkillID == 110035
			|| instance.nSkillID == 209035 
			|| instance.nSkillID == 210035))
	{
		instance.SendSkillFailed();
		return;
	}

	if (sTargetID != sCasterID 
		&& (instance.nSkillID == 109015 
			|| instance.nSkillID == 110015
			|| instance.nSkillID == 209015 
			|| instance.nSkillID == 210015))
	{
		instance.SendSkillFailed();
		return;
	}

	if (sCasterID >= NPC_BAND 
		|| sCasterID != pCaster->GetID())
		return;

	instance.sCasterID = (int16)sCasterID;
	instance.sTargetID = (int16)sTargetID;

	for (int i = 0; i < 7; i++)
		instance.sData[i] = sData[i];
		...
		if (pUser)
		{
			if (pSkill.bMoral >= MORAL_ENEMY)
			{
				if (byBuffType == BUFF_TYPE_SPEED || byBuffType == BUFF_TYPE_SPEED2)
					pUser->SendUserStatusUpdate(UserStatus::USER_STATUS_SPEED, UserStatusBehaviour::USER_STATUS_CURE);
				else if (byBuffType == BUFF_TYPE_STUN && !pTarget->isDebuffed())
				{
					pUser->SendUserStatusUpdate(UserStatus::USER_STATUS_POISON, UserStatusBehaviour::USER_STATUS_CURE);
				}
			}
			...
			if (bRemoveSavedMagic)
			{
				...
				if (byBuffType == (uint8)BuffType::BUFF_TYPE_BATTLE_CRY)
					pUser->SendItemMove(1, 1);
			}

			if (bRecastSavedMagic && TO_USER(pTarget)->isLockableScroll(pType->bBuffType))
			{
				pUser->SendUserStatusUpdate(USER_STATUS_POISON, USER_STATUS_CURE);
				pUser->RecastLockableScrolls(pType->bBuffType);
			}
		}
	}
