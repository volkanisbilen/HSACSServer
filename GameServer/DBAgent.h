...
#pragma region DailyReward
	uint8 LoadDailyReward(std::vector<ByteBuffer>& sItemList);
	uint8 LoadDailyCumulativeReward(std::vector<ByteBuffer>& sItemList);
	uint8 LoadDailyRewardUser(uint16 UserID, std::vector<ByteBuffer>& sItemList);
	uint8 UpdateDailyRewardUser(uint16 UserID, uint8 sData[], uint8 sDatatime[]);
#pragma endregion
#endif
#endif
...
#pragma region Clan System Methods
	...
	int8 KnightsCount(uint16 sClanID);
