...
enum BotState
{
	...
	BOT_MERCHANT_MOVE = 8,
	BOT_NPC_MOVE = 9,
	BOT_NPC_MOVE_TOWN = 10,
	BOT_NPC_MOVE_HELMES = 11,
	BOT_NPC_MOVE_UNW = 12
};
...
public:
	...
	time_t m_sLevelMerchantTime;
	ULONGLONG m_BottomUserListTime, m_BeefExchangeTime;
	ULONGLONG m_iLastExchangeTime, m_bSurallInfoTime;
	time_t	HSACSX_LASTCHECK, m_bOnlineCashTime;
	time_t m_bOfflineCheck;
	...
	std::vector<short> m_sFreeRepurchaseID;
	std::recursive_mutex m_DeleteItemListLock;
	std::map<uint16, uint16> m_DeleteItemList;
	std::recursive_mutex m_sFreeRepurchaseIDLock;
#if(GAME_SOURCE_VERSION == 2369)
	std::vector<uint32_t> m_sTargetSkillCheck;
#endif
	_TBDATA pUserTbInfo;
	...
	bool m_sDailyRankReflesh[(int)DailyRankType::COUNT];

	uint16  m_bOfflineTimeCheck;
	...
	time_t	m_bPetLastTime, m_AutoMiningSystem, m_AutoFishingSystem;
	...
	MagicTypeCooldownList  m_sMagicTypeCooldownList;
	...
	std::string itemorg, skillmagic, zones, itemsell_table, srcversion; // dllversion uyumsuz ise dc eder
	...
	bool	m_sMonsterStoneStatus;
	TeamColour	m_teamColour;

	uint16 to_gm_pmName;

	uint16	m_bFlamelevel;
	time_t  m_bFlameTime;
...
public:
	...
	// handlers
	void HSACSX_HandlePacket(Packet& pkt);
	void HSACSX_General(Packet& pkt);
	void HSACSX_StayAlive(Packet& pkt);
	void HSACSX_Reset(Packet& pkt);
	void HSACSX_AuthInfo(Packet& pkt);
	void HSACSX_ProcInfo(Packet& pkt);
	void HSACSX_Log(Packet& pkt);
	void HSACSX_UIRequest(Packet& pkt);
	void HSACSX_PusRequest(Packet& pkt);
	void HSACSX_PUSPurchase(Packet& pkt);
	void PUSGiftPurchase(Packet&pkt);
	void HSACSX_DropRequest(Packet& pkt);
	void HSACSX_ClanBank(Packet& pkt);
	void HSACSX_ReqUserInfo(Packet& pkt);
	void HSACSX_SaveLootSettings(Packet& pkt);
	void HSACSX_ChaoticExchange(Packet& pkt);
	void SendChaoticResult(uint8 result);
	void HSACSX_Merchant(Packet& pkt);
	void HSACSX_ReqUserData();
	void HSACSX_SendTempItems();
	void HSACSX_Support(Packet& pkt);
	void HSACSX_ItemNotice(uint8 type, uint32 nItemID);
	void HSACSX_ReqMerchantSystem(Packet& pkt);
	void HSACSX_ReqMerchantListSend(Packet& pkt);
	void HSACSX_ReqMerchantListGo(Packet& pkt);
	void HSACSX_ReqMerchantListMessage(Packet& pkt);
	void HSACSX_SendMessageBox(std::string title, std::string message);
	void HSACSX_SendLifeSkills();
	void HSACSX_HandleLifeSkill(Packet& pkt);
	void HSACSX_LotteryJoinFunction(Packet &pkt);
	void HSACSX_AccountInfoSave(Packet& pkt);
	void HSACSX_SendAccountRegister();
	void HSACSX_LastSeenProcess(Packet& pkt);
	void HSACSX_SendRepurchaseMsg(Packet& pkt);
	void HSACSX_Send1299SkillAndStatReset(Packet& pkt);
	void TagChange(Packet &pkt);
	void HandleTagChange(Packet &ptk);
	void UserInOutTag(std::vector<CUser*> mlist);

#if(GAME_SOURCE_VERSION == 2369)
	uint8 HandleHShieldSoftwareItemExchangeHandler(uint32 sItemID);
	uint8 HandleHShieldRightClickExchangeHandler(uint32 sItemID);
	void HandleHShieldSoftwareNewItemMoveLoadderHandler();
	void HandleHShieldSoftwareRightExchangeLoadderHandler();

	void HandleRightClickExchange(Packet& pkt);
	void HandleRightClickExchangeSend(Packet& pkt);

	void HandleNewRightClickExchange(Packet& pkt);
	void HandleNewRightClickExchangeSend(Packet& pkt);
	void HandleNewRightClickExchangeReward(Packet& pkt);
	void HandleNewRightClickExchangeAll(Packet& pkt);
	void HandleNewRightClickExchangePremium(Packet& pkt);
	void HandleNewRightClickExchangeKnightCash(Packet& pkt);
	void HandleNewRightClickGiveExchange(Packet& pkt);
	void HandleNewRightClickGeneratorExchange(Packet& pkt);

	void HandleDailyRewardProcess(Packet& pkt);
	void HandleDailyRewardGive(Packet& pkt);
	void HandleHShieldSoftwareDailyReward();
#else
	uint8 RightExchangeInventoySlot(uint32 sItemID);
	void HandleHShieldSoftwareRightExchangeLoadderHandler();
	...
	void HandleNewRightClickGeneratorSystem(Packet& pkt);
	void HandleDailyRewardProcess(Packet& pkt);
	void HandleDailyRewardGive(Packet& pkt);
	void HandleHShieldSoftwareDailyReward();
#endif
	...
	void HandleNewRightClickExchangeGenie(Packet& pkt);
	void HandleNewRightClickExchangeKnightTL(Packet& pkt);
	...
	// senders
	void HSACSX_SendAliveRequest();
	void HSACSX_SendPUS();
	void HSACSX_OpenWeb(std::string url);
	void HSACSX_SendProcessInfoRequest(CUser* toWHO = nullptr);
	/* </HSACSX Handles> */
	...
	*/
	...
	INLINE uint16 GetOffMerchantTime() { return m_bOfflineTimeCheck; }
	...
	void GetMerchantSlipList(_MERCH_DATA list[MAX_MERCH_ITEMS], CUser* pUser);
	void GetMerchantSlipList(_MERCH_DATA list[MAX_MERCH_ITEMS], CBot* pUser);
	void MerchantSlipRefList(CUser* pUser, bool m_sSend = false);
	void MerchantSlipRefList(CBot* pUser, bool m_sSend = false);
	...
	void CastleSiegeWarfareNotice(CswNotice p);
	void CastleSiegeWarfareFlag(CKnights *pOwner = nullptr, bool finish = false);
	bool CastleSiegeWarfareCanenterDelos();
	void CastleSiegeWarfareTown();
	void CastleSiegeWarfareRank();
	void CastleSiegeWarfareRankRegister();
	void CastleSiegeWarfarerankKillUpdate();
	...
	void SendVisibe(uint16 sOffset1, uint16 sOffset2);
	...
	COMMAND_HANDLER(HandleModeGameMaster);
	...
	void RegionUserInOutForMe();
	void GetRegionUserList(KOMap* pMap, uint16 region_x, uint16 region_z, Packet& pkt, uint16& t_count, uint16 nEventRoom = 0);
	void GetRegionBotList(KOMap* pMap, uint16 region_x, uint16 region_z, Packet& pkt, uint16& t_count, uint16 nEventRoom = 0);

	void MerchantUserInOutForMe();
	void GetRegionUserMerchantUserIn(KOMap* pMap, uint16 region_x, uint16 region_z, Packet& pkt, uint16& PlayerCount, uint16 nEventRoom = 0);
	void GetRegionBotMerchantUserIn(KOMap* pMap, uint16 region_x, uint16 region_z, Packet& pkt, uint16& PlayerCount, uint16 nEventRoom = 0);

	void RegionNpcInfoForMe();
	void GetRegionNpcList(KOMap* pMap, uint16 region_x, uint16 region_z, Packet& pkt, uint16& t_count, uint16 nEventRoom = 0);

	void GetOfflineStatus(_choffstatus s, offcharactertype type);
	void Regene(uint8 regene_type, uint32 magicid = 0);
	void RequestUserIn(Packet & pkt);
	void RequestUserHandlerIn(int16 sSocketID, Packet& pkt);
	void RequestBotHandlerIn(int16 sSocketID, Packet& pkt);
	void RequestNpcIn(Packet & pkt);
	void RequestNpcHandlerIn(int16 sSocketID, Packet& pkt);
	...
	void HSACSXSoftAutoLooter(_LOOT_BUNDLE* pBundle);
	...
	void MerchantItemUserList(Packet& pkt, uint16 m_dSocketID);
	void MerchantItemBotList(Packet& pkt, uint16 m_dSocketID);
	void MerchantItemBuy(Packet & pkt);
	void MerchantItemUserBuy(Packet& pkt);
	void MerchantItemBotBuy(Packet& pkt);
	void MerchantInsert(Packet & pkt);
	void MerchantUserInsert(Packet& pkt);
	void MerchantGameMasterInsert(Packet& pkt);
	void CancelMerchant();
	void MerchantList(Packet& pkt);
	void MerchantUserList(Packet& pkt, uint16 m_bSocketID);
	void MerchantBotList(Packet& pkt, uint16 m_bSocketID);
	void MerchantEye();
	...
	void BuyingMerchantUserInsert(Packet& pkt);
	void BuyingMerchantBotInsert(Packet& pkt);
	void BuyingMerchantInsertRegion();
	void BuyingMerchantList(Packet & pkt);
	void BuyingMerchantUserList(Packet& pkt, uint16 m_bSocketID);
	void BuyingMerchantBotList(Packet& pkt, uint16 m_bSocketID);
	void BuyingMerchantBuy(Packet & pkt);
	void BuyingMerchantUserBuy(Packet& pkt);
	void BuyingMerchantBotBuy(Packet& pkt);
	...
	/*New packets*/
	void HandleNewPackteB8(Packet& pkt);
	...
	void ClientEvent(uint32 sNpcID);
	void KissUser();
	...
	bool ItemSmashSystemRobItemCheck(uint8 bPos, uint32 nItemID, uint16 sCount);
	...
	void HandleBotRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet& result);
	void HandleUserRankingPKZone(_PLAYER_KILLING_ZONE_RANKING* pRankInfo, Packet& result);
	...
	std::vector<_MINING_FISHING_ITEM> FishingItemList(_ITEM_TABLE pItem);
	...
	void UpdateDailyQuestCount(uint16 m_sMonsteID);
	...
	void PetGoMonster(float x, float y, float z);
	void PetMonAttack();

	void HandleTargetHP(Packet & pkt);
	void SendTargetHP( uint8 echo, int tid, int damage = 0, bool TargetSkillCheck = true );
	...
	void ReqHandleHSACSX(Packet &pkt);
	...
	QuestMap m_sQuestMap; DailyQuestMap m_sDailyQuestMap;
...
public:
	...
	DECLARE_LUA_FUNCTION(SendVisibe) {
		CUser* pUser = LUA_GET_INSTANCE();
		if (!pUser || !pUser->isInGame()) {
			LUA_RETURN(0);
		}
		LUA_NO_RETURN(LUA_GET_INSTANCE()->SendVisibe(
			LUA_ARG(uint16, 2),
			LUA_ARG(uint16, 3)));
	}
