...
bool CUser::GetUnderTheCastleOpen() 
{ 
	return g_pMain->pUnderTheCastle.isActive;
}

uint16 CUser::GetUnderTheCastleUserCount() 
{ 
	Guard lock(g_pMain->pUnderTheCastle.UserListLock);
	return (uint16)g_pMain->pUnderTheCastle.UserList.size();
}
...
#if(__VERSION < 2369)
	Packet result(WIZ_HSACS_HOOK, uint8(HSACSXOpCodes::JURAID));
