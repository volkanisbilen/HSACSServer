...
void CNpc::HpChange(int amount, Unit* pAttacker /*= nullptr*/, bool isDOT /*= false*/)
{
	...
	if (pAttacker != nullptr && pAttacker->isPlayer())
		TO_USER(pAttacker)->SendTargetHP(0, GetID(), amount, false);
...
void CNpc::RecvAttackReq(int nDamage, uint16 sAttackerID, AttributeType attributeType /*= AttributeNone*/)
{
			...
			if (pNpc)
			{
				...
				else
				{
					if (MyDamage > 0)
					{
						...
						m_damagenpcListLock.lock();
						auto itr = m_DamagedNpcList.find(socket_id);
						if (itr != m_DamagedNpcList.end())
						{
							if (UNIXTIME - itr->second.lastdamagedt > 300)
								itr->second.Reset();
						...
						{
							m_DamagedNpcList.insert(std::make_pair(socket_id, __NpcDamagedList(MyDamage, UNIXTIME)));
						}
						m_damagenpcListLock.unlock();
					}
				}
	...
	if (GetNpcState() != (uint8)NpcState::NPC_FAINTING)
	{
		...
		else 
		{
			...
			else if (pAttacker->isBot())
				ChangeTarget(0, TO_BOT(pAttacker));
		}
	}
}
...
void CNpc::OnDeathProcess(Unit* pKiller)
{
	...
	if (!m_bMonster)
	{
		...
		case NPC_DESTROYED_ARTIFACT:
			CastleSiegeWarfareMonumentProcess(pUser);
			break;
	...
	else
	{
		...
		if (g_pMain->m_MonsterResourceArray.GetSize() > 0)
		{
			...
			if (pMonsterResource)
			{
				...
				result << pUser->GetNation() << uint32(pUser->GetSocketID()) << uint8(0) << strResource;
		...
		{
			if (g_pMain->pUnderTheCastle.isActive)
		...
		{
			if (pUser->m_sMonsterStoneStatus)
...
void CNpc::OnRespawn()
{
	...
	if (GetZoneID() == ZONE_UNDER_CASTLE)
	{
		if (g_pMain->pUnderTheCastle.isActive)
		{
			...
			g_pMain->pUnderTheCastle.m_UnderTheCastleMonsterList.insert(std::make_pair(GetID(), GetProtoID()));
...
void CNpc::StateChangeServerDirect(uint8 bType, uint32 nBuff)
{
	...
	result << uint32(GetID()) << bType << nBuff;
	SendToRegion(&result);
}
/*
...
void CNpc::Dead(Unit* pKillerUnit /*= nullptr*/, bool bSendDeathPacket /*= true*/)
{
	...
	m_HSACSXSoftDelay = m_sRegenTime + UNIXTIME2;
	m_bFirstLive = false;
	m_byDeadType = 100;
	m_iHP = 0;
...
bool CNpc::IsCloseTarget(CBot* pUser, int nRange)
{
	if (pUser == nullptr
		|| pUser->isDead()
		|| !isInRangeSlow(pUser, nRange * 2.0f))
...
void CNpc::ChangeTarget(int nAttackType, CBot* pUser)
{
	if (isDead())
		return;
	...
	CBot* preUser = nullptr;
	if (hasTarget() && m_Target.id > MAX_USER && m_Target.id < NPC_BAND)
		preUser = g_pMain->GetBotPtr(m_Target.id);
...
void CNpc::TracingAttack()
{
	...
	if (nID < MAX_USER)	// Target is a player
	{
	...
	else if (nID > MAX_USER && nID < NPC_BAND)
	{
...
int CNpc::GetTargetPath(int option)
{
	...
	m_fSecForMetor = m_fSpeed_2;
	CUser* pUser = nullptr; CNpc* pNpc = nullptr; CBot* pBot = nullptr;
	...
	// Bot YNG Ekledi Copy Paste
	else if (m_Target.id > MAX_USER && m_Target.id < NPC_BAND)
	{
		pBot = g_pMain->GetBotPtr(m_Target.id);
		if (pBot == nullptr
			|| !pBot->isInGame()
			|| pBot->isDead()
	...
	else if (m_Target.id >= NPC_BAND && m_Target.id < INVALID_BAND)
	{
		...
		if (pNpc->m_iHP <= 0 || pNpc->GetNpcState() == (uint8)NpcState::NPC_DEAD)
		{
			InitTarget();
			return -1;
		}
...
bool CNpc::IsDamagedNpcList(CNpc* pNpc)
{
	if (pNpc == nullptr)
		return false;

	uint16 socket_id = pNpc->GetID();
	Guard lock(m_damagenpcListLock);
	auto itr = m_DamagedNpcList.find(socket_id);
	return itr != m_DamagedNpcList.end() && itr->second.Damage && ((UNIXTIME - itr->second.lastdamagedt) < 500);
}
...
bool CNpc::IsDamagedBotList(CBot* pUser)
{
	if (pUser == nullptr)
		return false;

	uint16 socket_id = pUser->GetID();
	Guard lock(m_damageuserListLock);
	auto itr = m_DamagedUserList.find(socket_id);
	return itr != m_DamagedUserList.end() && itr->second.Damage && ((UNIXTIME - itr->second.lastdamagedt) < 500);
}
...
void CNpc::SendAttackRequest(int16 tid)
{
	...
	if (sDamage > 0)
	{
		...
		if (pTarget->isDead()) 
			bResult = (uint8)AttackResult::ATTACK_TARGET_DEAD;
		else 
			bResult = (uint8)AttackResult::ATTACK_SUCCESS;

		if (pTarget->isPlayer())
		{
			if (!TO_USER(pTarget)->isInGenie())
				TO_USER(pTarget)->ItemWoreOut(DEFENCE, sDamage);
		}
	}
	Packet result(WIZ_ATTACK, uint8(LONG_ATTACK));
	result << bResult << uint32(GetID()) << uint32(tid);
	SendToRegion(&result);
}
...
bool CNpc::SetLive()
{
	...
	if (!bos_check)
	{
		...
		else
		{
			...
			{
				TRACE("HSACSXSoft - Dominik | #### Npc-SetLive() Fail : nTileX=%d, nTileZ=%d #####\n", nTileX, nTileZ);
				return false;
			}
...
float CNpc::FindEnemyExpand(int nRX, int nRZ, float fCompDis, UnitType unitType)
{
	...
	if (unitType == UnitType::UnitPlayer)
	{
		...
		foreach_region(x, z)
		{
			...
			foreach(itr, cm_RegionUserArray)
			{
				...
				if ((isGuard() && pUser->isInMoradon())
					|| (!isMonster() && GetByGroup() == Nation::NONE))
					continue;
				...
				if (m_tNpcAttType == ATROCITY_ATTACK_TYPE)
				{
					if (pUser->isTransformed())
					{
						if (pUser->isInPKZone())
						{
							...
							else if (m_Target.id == target_uid)
							{
								m_Target.id = target_uid;
								m_Target.bSet = true;
								m_Target.x = pUser->GetX();
	...
	else if (unitType == UnitType::UnitBot)
	{
		...
		foreach_region(x, z)
		{
			...
			ZoneUserArray cm_RegionBotsArray = pRegion->m_sRegionBotArray;
			pRegion->m_sBotArrayLock.unlock();
			foreach(itr, cm_RegionBotsArray)
			{
				...
				float fDis = GetDistanceSqrt(pBot);
				if (fDis > fSearchRange
					|| (fComp > 0 && fDis > fComp))
					continue;
				...
				if ((m_tNpcAttType == ATROCITY_ATTACK_TYPE && !pBot->isTransformed())
					|| (m_tNpcAttType == TENDER_ATTACK_TYPE && ((m_bHasFriends && m_Target.id == target_uid))))
				{
					m_Target.id = target_uid;
					m_Target.bSet = true;
					m_Target.x = pBot->GetX();
...
bool CNpc::FindEnemy()
{
	...
	//Bot Sistem Eklendi YNG Copy Paste
	if (isMonster() || (!GetMap()->areNPCsFriendly() || GetNation() != Nation::ALL))
	{
...
void CNpc::MoveAttack()
{
	...
	else if (m_Target.id > MAX_USER && m_Target.id < NPC_BAND)
	{
		...
		if (pBot == nullptr || (m_Target.m_tLastDamageTime > 0 && (UNIXTIME2 - m_Target.m_tLastDamageTime > 6000)))
		{
			InitTarget();
			return;
		}
		vUser.Set(pBot->GetX(), pBot->GetY(), pBot->GetZ());
...
void CNpc::SendMoveResult(float fX, float fY, float fZ, float fSpeed /*= 0.0f*/)
{
	...
	result << uint8(1) << uint32(GetID()) << GetSPosX() << GetSPosZ() << GetSPosY() << uint16(fSpeed * 10);
	SendToRegion(&result);
}
...
void CNpc::SendExpToUserList()
{
	...
	uint32 nMaxDamage = 0;
	std::map<CUser*, uint32> filteredDamageUserList;
	...
	foreach(itr, m_copyUserlist)
	{
		...
		if (pUnit->isPlayer())
		{
			...
			CUser* pUser = g_pMain->GetUserPtr(pUnit->GetID());
			if (pUser == nullptr 
			...
			if (!pUser->isInParty())
			{
				auto filteredDamageListItr = filteredDamageUserList.find(pUser);
				if (filteredDamageListItr == filteredDamageUserList.end())
					filteredDamageUserList.insert(std::make_pair(pUser, itr->second.Damage));
				else
					filteredDamageUserList[pUser] = itr->second.Damage;
			...
			else
			{
				...
				filteredDamageUserList[partyItr->second] += itr->second.Damage;
			}
		}
		else if (pUnit->isNPC())
		{
			CNpc* pNpc = g_pMain->GetNpcPtr(pUnit->GetID(), GetZoneID());
			if (pNpc == nullptr
				|| !pNpc->isPet()
			...
			if (filteredDamageListItr == filteredDamagePetList.end())
				filteredDamagePetList.insert(std::make_pair(pNpc, itr->second.Damage));
			else
				filteredDamagePetList[pNpc] = itr->second.Damage;
		}
	}

	if (!filteredDamageUserList.empty())
	{
		foreach(itr, filteredDamageUserList)
		{
	...
	if (!filteredDamagePetList.empty())
	{
		foreach(itr, filteredDamagePetList)
		{
			CNpc* pUser = itr->first;
			if (pUser == nullptr
				|| !pUser->isPet())
				continue;

			pUser->RecvPetExp(this, itr->second, m_TotalDamage, m_proto->m_iExp, m_proto->m_iLoyalty);

			if (itr->second > nMaxDamage)
			{
				m_sMaxDamageUserid = pUser->GetPetID();
				strMaxDamageUser = pUser->GetNameUser();
				nMaxDamage = itr->second;
			}
		}
	}
...
void CNpc::GiveNpcHaveItem(Unit* pKiller)
{
	...
	if (isMonster())
	{
		...
		if (pItem != nullptr)
		{
			for (int j = 0; j < LOOT_DROP_ITEMS; j++)
			{
				...
				if (pUser->m_bFlamelevel > 0 && pUser->m_bFlamelevel <= 3 && g_pMain->pBurningFea[pUser->m_bFlamelevel - 1].droprate)
					iPer += iPer * g_pMain->pBurningFea[pUser->m_bFlamelevel - 1].droprate / 100;
	...
	result << uint32(GetID()) << pBundle->nBundleID << uint8(1);
	if (!pUser->isInParty()) pUser->Send(&result);
	else g_pMain->Send_PartyMember(pUser->GetPartyID(), &result);


	//Robin Loot Begin
	bool m_sRobinLooting = false;
	//Pet Loot Begin
	bool m_sPerlootingAktive = false;

	if (!pUser->isInParty())
	{
		if (pUser->m_bAutoLoot)
			m_sRobinLooting = true;
		else if (pUser->m_PettingOn != nullptr)
		{
			if (pUser->m_PettingOn->sStateChange == MODE_LOOTING)
				m_sPerlootingAktive = true;
		}
	}
	else if (pUser->isInParty())
	{
		...
		if (pParty != nullptr)
		{
			...
			for (int i = 0; i < MAX_PARTY_USERS; i++)
			{
				...
				if (PartyUser == nullptr
					|| !isInRangeSlow(PartyUser, MAX_LOOT_RANGE * 2))
					continue;

				if (PartyUser->m_bAutoLoot)
					m_sRobinLooting = true;
				else if (PartyUser->m_PettingOn != nullptr)
				{
					if (PartyUser->m_PettingOn->sStateChange == MODE_LOOTING)
						m_sPerlootingAktive = true;
				}

				if (m_sRobinLooting)
				{
					PartyUser->HSACSXSoftAutoLooter(pBundle);
					break;
				}
				else if (m_sPerlootingAktive)
				{
					PartyUser->LootingPet(GetX(), GetZ());
					break;
				}
			}
		}
	...
	if (m_sRobinLooting)
	{
		pUser->HSACSXSoftAutoLooter(pBundle);
		return;
	}
	else if (m_sPerlootingAktive)
	{
...
void CNpc::StateChange(uint8 bState)
{
		...
		&& bState != (uint8)NpcState::NPC_SLEEPING)
	{
		...
		result << uint32(GetID()) << uint8(1) << uint32(5) << uint32(0);
		SendToRegion(&result);
		m_tFaintingTime = 0;
	}
...
void CNpc::NoticeRespawn(std::string& Monster)
{
	if (!Monster.empty())
	{
		...
		sMonSummonNotice = string_format("The GM %s has summoned the %s in %s position(%d,%d).", g_pMain->m_sSummonGameMaster.c_str(), Monster.c_str(), sZoneName.c_str(), uint16_t(GetX()), uint16_t(GetZ()));
		g_pMain->SendNotice(sMonSummonNotice.c_str(), (uint8)Nation::ALL);
		g_pMain->m_sSummonMessage = false;
		g_pMain->m_sSummonGameMaster.clear();
	}
	else
	{
		TRACE("the npc have not name\n");
		g_pMain->m_sSummonMessage = false;
		g_pMain->m_sSummonGameMaster.clear();
	}
}
...
void CNpc::SantaDeath()
{
	if (GetProto() == nullptr
		|| GetProto()->m_area_range < 1.0f
		|| GetType() != NPC_SANTA)
		return;
	...
	for (uint16 i = 0; i < MAX_USER; i++)
	{
		...
		if (pUser->isDead()
			|| !pUser->isInRangeSlow(GetX(), GetZ(), GetProto()->m_area_range))
			continue;

		if (GetProto()->m_iExp)
			pUser->ExpChange("New Years Event", GetProto()->m_iExp, true);
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (itemid && itemcount)
				pUser->GiveItem("New Years Event", itemid, itemcount);
		}
	}
}