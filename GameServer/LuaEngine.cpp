...
DEFINE_LUA_FUNCTION_TABLE(g_globalFunctions,
	...
	MAKE_LUA_FUNCTION(GiveSwitchPremium) //17.12.2020 Switch Premium Yukler Yuklemez 3 ünü birden goruntuluyecek sekilde yeni komut eklendi
...
bool CLuaEngine::ExecuteScript(CUser * pUser, CNpc * pNpc, int32 nEventID, int8 bSelectedReward, const char * filename)
{
	...
	if (itr == m_scriptMap.end())
	{
		...
		sNotice << pUser->GetNation() << uint32(pUser->GetSocketID()) << uint8(0) << sNoticeMessage;
