...
CKnights::CKnights()
{
	...
	m_sLadderPointList.clear();
	m_sLadderPointTime = 0;
	m_bOnlineNembers = 0;
	m_bOnlineNpCount = 0;
	m_bOnlineExpCount = 0;
...
void CKnights::RefundDonatedNP(uint32 nDonatedNP, CUser * pUser /*= nullptr*/, const char * strUserID /*= nullptr*/)
{
	...
	if (pUser != nullptr)
	{
		...
		if (!pUser->RobItem(ITEM_CONT_RECOVERY)	&& !pUser->RobItem(ITEM_CONT_RESTORE_CERT))
...
void CKnights::Disband(CUser *pLeader /*= nullptr*/)
{
 ...
 string clanNotice;
	...
	std::vector<std::string> m_sKnightUser;
	...
	foreach(i, m_sKnightsUser)
	{
		...
		m_sKnightUser.push_back(i->second->strUserName);
	}

	foreach(i, m_sKnightUser)
	{
	...
	if (pLeader != nullptr)
	{
		...
		if (pLeader->isKing()) 
		{
			...
			if (pLeader->GetNation() == (uint8)Nation::ELMORAD) 
				capes << uint16(KingCapeType::KNIGHTS_HUMAN_KING_CAPE);
			else 
				capes << uint16(KingCapeType::KNIGHTS_KARUS_KING_CAPE);
...
void CGameServerDlg::UpdateFlagAndCape()
{
	...
	{
		if (itr->second->m_byFlag >= (uint8)ClanTypeFlag::ClanTypeAccredited5) 
			itr->second->m_byGrade = 1;
		else 
			itr->second->m_byGrade = g_pMain->GetKnightsGrade(itr->second->m_nPoints);
