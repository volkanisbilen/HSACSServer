...
void CUser::OnDeathKilledPlayer(CUser* pKiller)
{
	...
	if (pKiller != this)
	{
		...
		if (m_party_check)
		{
			...
			if (pKiller->isInParty() 
				&& (pParty = g_pMain->GetPartyPtr(pKiller->GetPartyID())) != nullptr)
			{
...
void CUser::OnDeathKilledNpc(CNpc* pKiller)
{
	...
	result << uint8_t(LONG_ATTACK) << uint8_t(AttackResult::ATTACK_TARGET_DEAD) << (uint32)pKiller->GetID() << (uint32)GetID();
	Send(&result);*/
...
devamet:
	...
	if (pktsend) { newpkt << (uint16)++pAssistKill.serikillcount; Send(&newpkt); }
