...
public:
	...
	static void Construct(Packet * result, uint8 bType, std::string * strMessage = nullptr, std::string * strSender = nullptr, 
		uint8 bNation = 1, int16 senderID = -1, int8 personelrank = 0, uint8 m_sAuthority = 0, uint8 systemmsg = 0)
	{
		result->Initialize(WIZ_CHAT);
		*result << bType << bNation << uint32(senderID);
		...
		*result << personelrank << m_sAuthority << systemmsg;
	}

	static void Construct(Packet * result, uint8 bType, const char * szMessage, const char * szSender = "",
		uint8 bNation = 1, int16 senderID = -1, int8 personelrank = -1, uint8 m_sAuthority = 0, uint8 systemmsg = 0)
	{
		std::string strSender = szSender, strMessage = szMessage;
		Construct(result, bType, &strMessage, &strSender, bNation, senderID, personelrank, m_sAuthority, systemmsg);
	}
};
