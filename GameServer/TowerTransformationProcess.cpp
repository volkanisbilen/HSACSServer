...
void CUser::TowerExitsFunciton(bool Dead)
{
	...
	if (Dead) {
		...
		result << uint8(1) << uint32(GetSocketID()) << uint32(pNpc->GetID());
		Send(&result);
...
void CUser::HandleTowerPackets(Packet & pkt)
{
	...
	case 16:
	{
		...
		result << command << uint8(1) << uint32(GetID()) << uint32(pNpc->GetID())
			<< pNpc->GetSPosX() << pNpc->GetSPosZ() << pNpc->GetSPosY();
		Send(&result);
	}
	break;
	case 17:
	{
		...
		InitType4();
		RecastSavedMagic();
		result << command << uint8(1) << uint32(GetID());
		Send(&result);
	}
	break;
	default:
...
void CUser::ChangeFame(uint8 bFame)
{
	...
	m_bFame = bFame;
	result << uint32(GetSocketID()) << GetFame();
	SendToRegion(&result);
}