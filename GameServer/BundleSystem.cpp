...
void CUser::HSACSXSoftAutoLooter(_LOOT_BUNDLE* pBundle) {
	if (m_bFairyCheck || !pBundle)
		return;
	...
	C3DMap* pMap = GetMap();
	if (pMap == nullptr || !pMap->m_bAutoLoot || !pBundle->ItemsCount || (!m_bAutoLoot && !isInParty())) 
		return;
	...
	if (!m_bAutoLoot) {
		...
		for (int i = 0; i < MAX_PARTY_USERS; i++) {
			...
			if (pUser == nullptr || !pUser->m_bAutoLoot || pUser->isDead() || !isInRange(pBundle->x, pBundle->z, RANGE_50M)) continue;
			pLooter = pUser;
			break;
		}
		if (!pLooter) 
	...
	for (int i = 0; i < NPC_HAVE_ITEM_LIST; i++) {
		...
		if (pBundle->Items[i].nItemID == ITEM_GOLD) {
			...
			pReceiver->HandleHShieldSoftwareNewItemMoveLoadderHandler();
#endif
