...
void CUser::MiningExchange(uint16 ID)
{
	...
	auto m_sMiningExchangeArray = g_pMain->m_MiningExchangeArray.m_UserTypeMap;
	g_pMain->m_MiningExchangeArray.m_lock.unlock();

	if (m_sMiningExchangeArray.empty())
		return;

	foreach(itr, m_sMiningExchangeArray)
	{
	...
	if (MiningExchangeList.size() > 0)
	{
		...
		foreach(str, m_sMiningExchangeArray)
		{
		...
		if (pMiningExchangeEffect->GiveEffect == 1)
		{
			...
			x << resultCode << uint32(GetID()) << sEffect;
			Send(&x);
		}
	}
}
