...
public:
	...
	bool runnable()
	{
		...
		while (m_opened && m_threadRunning)
		{
			...
			if (m_sClientSocket->m_threadPort == 15001 || m_sClientSocket->m_threadPort == 15100)
			{
			...
			Socket* socket = m_sClientSocket->AssignSocket(aSocket, m_tempAddress);		// Idle to active sessions
