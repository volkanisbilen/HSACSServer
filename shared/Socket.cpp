...
Socket::Socket(SOCKET fd, uint32 sendbuffersize, uint32 recvbuffersize)
	: m_fd(fd), m_connected(false), m_deleted(false), m_sClientSocket(nullptr), m_sOfflineSystem(false)
{
	// Allocate buffers
...
bool Socket::Connect(const char * Address, uint32 Port)
{
	...
	m_completionPort = m_sClientSocket->GetCompletionPort();
...
void Socket::_OnConnect()
{
	...
	m_sClientSocket->OnConnect(this);
...
void Socket::Disconnect(bool m_sSocketDisconnect /*= false*/)
{
	...
	{
		if (!IsGetSystem())
			m_sOfflineSystem = true;

		if (m_sSocketDisconnect)
			return;
