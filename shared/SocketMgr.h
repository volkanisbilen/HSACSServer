...
public:
	...
	INLINE bool isDesyncPacket() { return m_sOpenDesyncPacket; }
...
protected:
	...
	INLINE void IncRef() { if (m_sRefCounter.increment() == 1) SetupSockets(); }
	INLINE void DecRef() { if (m_sRefCounter.decrement() == 0) CleanupSockets(); }

	// reference counter (one app can hold multiple socket manager instances)
	static Atomic<uint32> m_sRefCounter;
