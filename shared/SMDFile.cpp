...
{
	m_WarpArray.m_lock.lock();
	auto m_sWarpArray = m_WarpArray.m_UserTypeMap;
	m_WarpArray.m_lock.unlock();

	foreach(itr, m_sWarpArray)
	{
		_WARP_INFO* pWarp = itr->second;
		if (pWarp == nullptr 
			|| (pWarp->sWarpID / 10) != warpGroup) 
			continue;
...
bool SMDFile::IsValidPosition(float x, float z, float y) { return (x < m_N3ShapeMgr->Width() && z < m_N3ShapeMgr->Height()); }
